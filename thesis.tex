...
\subsubsection{Replay}\label{di:prb}
...
In the per-dataset replay phase, we apply the unsynced changes that are encoded in the entries of a particular log to its dataset.

\subsubsection{Replay Sequence Algorithm}
The basis for claiming and replay is the \textit{replay sequence algorithm}.
It consists of a \textit{scanning phase} and a \textit{iteration phase}.
The scanning phase uses the traversal algorithm (Section~\ref{di:prb:traversal}) on a provided set of chunks to search for log entries that have the HDL's \textit{log GUID}.
It wraps these candidate entries in a \textit{replay node} and collects the nodes in a b-tree.
In the \textit{iteration phase}, the algorithm \textit{visits} each replay node by iterating over the b-tree.
For each node, it checks whether the corresponding entry needs to be replayed and if so, invokes a user-defined callback.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    ...
    \item[Pointer To Chunk] A pointer to the \lstinline{prb_chunk_t} that contains the entry.
\end{description}
...
The iteration phase keeps all state in the \textit{replay progress} structure which contains the following variables:
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    \item[$\mathbf{(gen_{last}, gsid_{last})}$] The $gen$ and $gsid$ of the last visisted replay node.
        A value of $(0,0)$ means that no replay node has been visited yet.
    \item[precrash-txg] The last transaction group that was completely synced before the system crashed.
...
\end{description}
% The purpose of the replay progress structure is twofold:
% First, itmust ensure that the callback is invoked exactly once for each unreplayed entry.
%  unreplayed entries are visisted  those entries twofold: it keeps track of already visisted entries during iteration, and to determine whether an entry needs to be skipped or replayed.
The purpose of the replay progress structure is to keep track of already visisted entries and the logical dependencies between them.
Remember from Section~\ref{di:prb:structure} that an entry $E$ in generation $E.gen$ depends on all entries in all generations $gen < E.gen$, and that an entry must only be replayed if the changes of its transaction group $E.txg$ have not been applied to the dataset.
Thus, when visiting the replay node that represents entry $E$, we skip it if its $E.txg \le precrash\text{-}txg$ or $(E.gen,E.gsid) \le (gen_{last}, gsid_{last})$.
Otherwise, we check whether all changes that it logically depends on have already been applied to the dataset using the \textit{validating counter tables}.
If that is the case, we update $(gen_{last}, gsid_{last}) = (E.gen, E.gsid)$ and invoke the callback.
If the callback returns an error or the check for dependencies fails, the algorithm stops and returns $E$ as a \textit{witness} for a missing entry or failed replay.

The \textit{validating counter tables} are our mechanism to check that logical dependencies are met before replaying an entry $E$.
For every entry that is not skipped, we update the validating counter tables in the same way as the write path (Section~\ref{di:prb:write:logstructureencoding}).
Then, we verify that the Counter Table stored in $E$ matches the \textit{last table} in the validating counter table.
In more detailed terms, the following steps are taken for each entry that is not skipped:
\begin{enumerate}[noitemsep]
    \item If $E.gen \neq gen_{last}$, compute a new \textit{last table} from the \textit{live table} state.
    \item Increment the \textit{live table} counter for txg $E.txg$.
    \item Compare the \textit{last table}'s counters to the Counters Table stored in $E$.
        Note that we must only compare the counters for $txg > precrash\text{-}txg$ since we skipped all entries in $txg \le precrash\text{-}txg$.
    \item If the counters match, we know that we have not lost any entries in generations $< E.gen$.
        Thus, all logical dependencies of $E$ are satisifed and we can invoke the callback.
    \item If the counters do not match, we know that the log has been corrupted.
        Under the assumption that entries can only be lost but not appear, we can even state that at least one entry has been in the previous generation.
        We stop the iteration and return an error with $E$ as a witness.
\end{enumerate}
A number of critical properties of the overall algorithm results from this counter validation step:
\begin{enumerate}[noitemsep]
...
The correctness of the replay sequence algorithm and the validation step in particular depends on the assumption that log entries can only disappear but never appear after a system crash.
An entry \textit{disappears} if it is unreachable by the traversal algorithm, either because the entry itself or an entry earlier in the chunk's sequence was corrupted.
An entry can only \textit{appear} in the b-tree if all of the following conditions hold:
\begin{itemize}[noitemsep]
...
\end{itemize}
If such a false entry appears in the b-tree, the behavior is implementation-dependent and likely leads to data corruption if the callback replays the (likely garbage\todo{language}) entry.
Whereas we believe that the conditions are sufficiently high hurdles for random data corruption, the following scenarios may also cause entries to appear:
\begin{itemize}[noitemsep]
\item Implementation errors on the write path or in the traversal algorithm. Without formal verification, we cannot protect against this.
\item Deliberate injection of log entries by a malicious party with write access to PMEM.
    This is out of scope for unencrypted datasets, but important for OpenZFS Native Encryption which authenticates and encrypts the data stored in datasets.
    While encryption support is out of scope for this thesis, we believe that forgery of entries can be prevented by authenticating the entry header.
...
%%A number of unlikely circumstances can cause collisions in the b-tree, i.e., replay nodes with the same $(gen, gsid)$:
%%\begin{enumerate}[noitemsep]
%%    \item Overflow of $gen$ or $gsid$. We have ruled this out, see Section~\ref{di:prb:persistent:entry}.
%%    \item Implementation error on the write path. We cannot protect against this.
%%    \item Undetected data corruption. We cannot protect against this, except by using stronger checksums which comes at a performance cost.\todo{explore?}
%%    \item A malicious party that deliberately\todo{gezielt...?} modifies entries in PMEM. This is only relevant for encryption which is out of scope for this thesis (see Section~\ref{sec:requirements}).
%%    \item Reuse of \textit{log GUIDs}. This is very unlikely because we generate them as 128 bit random numbers.
%%\end{enumerate}
%%We detect collisions during the scanning phase of the algorithm when inserting the second entry with the same identity into the b-tree.
%%In that case, we abort execution of the the algorithm immediately with an error.
%%Until measures that systematically prevent log GUID reuse are implemented, this is the safer approach compared to allowing iteration until the collision.
%%\todo{impl currently does this, but I thought about this during writing and it's not a good idea}
%%The reason is that accidental replay of an entry for a foreign dataset would most certainly, and possibly silently, corrupt the dataset whose log is being replayed.
%%\todo{future work section on log guid reuse, see latex comment here}
%% Future work could mitigate the risk of log GUID collision in PMEM as follows:
%%  1. Maintain a hashset of active log guids, let's call id H_a.
%%  2. Newly issued log guids must not be in H_a but are added to it once they are issued.
...
% constructs replay sequence and performs dry-run to determine which chunks need to be held back
% =>  refcount, wait replay list, HDL's held chunks list (ref chunk ownershiup section)
% => early detection of missing entries (pool import, reference api overview section)

\subsubsection{Crash Consistent Replay}
