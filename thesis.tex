...
The conceptual model for the log that PRB consumers write through a PRB is that of a \textbf{sequence of generations of log entries}.
\begin{description}
    \item[Log Entry] A log entry is an opauqe variable-length data blob provided by the PRB API consumer.
        Along with data blob, the log writer must provide the transaction group in which the entry becomes obsolete.
        It must also indicate whether the entry should start a new generation or whether it should be added to the currently open generation.
...
After a system crash the log chain can be replayed by providing a callback and the last transaction group that was successfully synced to the zpool before the system crashed (:=\~\textit{precrash-txg}).
The callback is invoked for \textbf{exactly} those log entries that have been successfully logged to PRB but not synced to the zpool before the system crashed.
The invocations happen sequentially and in a deterministic order that respects the logical dependencies expressed through the generations.
...
The emphasis on the word "exactly" in the previous paragraph is critical because log entries are not idempotent --- the effect encoded in the log entry must be applied exactly once.
This has two critical implications for replay:
...
We address this problem by remembering We use the transaction group provided with each entry and the to decide this second 


have already synced   since the changes encoded in entries for txgs $\le$ precrash-txg 
  that have not yet been synced newer than entries that encode changes for txgs   which have already been synced from txgs that have  that are already the txg is invoked so that  in the zpool so First, if the callback successfully persisted an entry the PRB implementation must not invoke the callback multiple times must 
Also, the effe since the ZIL and txg sync 

The purpose of replay is to apply \textbf{exactly} those changes to the filesystem or ZVOL dataset that were successfully logged to PRB but not synced to the zpool before the system crashed.
We emphasize the word "exactly" because we do not require to be idempotent.

that were successfully logged  to the ZIL and not yet logged bring the filesystem into the state that was committed is to catch up the zpool's last synced state transaction group and the state that was reported committed.
(See Section X.Y.Z\todo{ref} for an introduction  a conceptual model )


The goal of replay is tocatch up the main pool state  reco advance the system state committed  catch  to recover system state.
The input to the replay algorithm is the last-synced transaction group 
 pool's last synced transaction callbac
The replay procedure is callback-driven: the API consumer provides a callback function that is invoked by the replay algorithm for every entry that  that is invoked for every entry whose effect has not yet been applied to the pool.

after a system crash.
Log chains can be replayed after a system crash.
The replay is always sequential and fully deterministic. regardless of whether  regardless   Regardless of the shape Log chains can be replayed  can 
