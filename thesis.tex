...
Its input is the HDL's log GUID, a set of \lstinline{prb_chunk_t} references, a mutable reference to \textit{replay state}, and a callback.
The algorithm consists of a \textit{scanning phase} and a \textit{iteration phase}.
The scanning phase uses the traversal algorithm (Section~\ref{di:prb:traversal}) on the chunks to search for log entries that have the HDL's \textit{log GUID}.
...
The iteration phase needs to keep track of already visisted entries and logical dependencies between them.
It uses the following state to accomplish this task:
 \begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
...
\end{description}
% The purpose of the replay state structure is twofold:
...
The state of the iteration phase needs to be recoverable after a crash because replay must be crash-tolerant.
