...
PRB implements a generic logical write-ahead log facility that uses PMEM for persistence.
...
We present PRB in a top-down manner, starting with the logical structure of a single log in Section~\ref{di:prb:structure}, followed by a walkthrough of the API that PRB exposes towards \lstinline{zil\_pmem.c} (Section~\ref{di:prb:api}).
Equipped with a black-box understanding of PRB's functionality we proceed with its internal design and implementation.
In Section~\ref{di:prb:phys} we describe PRB's persistent data structure.
The design and implementation techniques of the scalability-oriented the write path are presented in Section~\ref{di:prb:write}.
Finally, Section~\ref{di:prb:recovery} describes\todo{word} the algorithms used on the recovery path (\ref{di:prb:recovery}).



\subsection{Conceptual Model}\label{di:prb:structure}
The conceptual model for the PRB is a PMEM resource manager that exposes the HDL abstraction.

The conceptual model for HDL is a logical write-ahead log for a dataset $D$.
It serves as persistent storage for committed changes to the dataset that have not yet been synced out to the main zpool.
The log can be opened once and written until it is either destroyed by the log writer or the system crashes.
After a system crash, the changes that have not yet been persisted to the main pool are applied again through replay.

The unit that is written to the log is the \textit{log entry}.
A log entry is a logical change $C$ to the dataseet $D$ that was made in a single transaction group $C_{txg}$.
The consumer of PRB encodes $C$ as an opaque blob and passes it to its HDL's \lstinline{write_entry} method.

PRB requires the log writer to specify the \texit{logical dependencies} of a log entry.
Logial dependencies are necessary for ordering and correctness during replay.
For example, if an application creates a file and writes to it, the log entry that encodes the write to the file depends on the log entry that encodes the creation of the file.
The dependency model that PRB exposes is based on \textit{generations}.
Entries within the same generation do not depend on each other.
But an entry from a newer generation depends on all log entries in all previous generations.
When an entry is written, the writer must indicate whether the entry should be placed in a new generation or whether the existing \textit{open generation} should be used.

After a system crash, the zpool is in the state of the last synced transaction group, the \textit{precrash-txg}.
The changes that need to be applied to the dataset $D$ are exactly those changes $C_i$ for which $C_i_{txg} > \text{precrash-txg}$ holds true.
%We store $C_i_{txg}$ in the metadata of the entry $E_i$ that represents change $C_i$.
To apply those changes, the PRB consumer provides the value of precrash-txg and a callback to the \textit{replay algorithm}.
The replay algorithm determines the set of $C_i$ that need to be replayed and determines a replay sequence that respects the entries' logical dependencies.
It then invokes the callback for each entry in the replay sequence.
The callback must perform the following steps in an atomic DMU transaction for each unapplied entry $E$.
\begin{enumerate}[noitemsep]
    \item Apply the change that the log writer encoded encoded in the entry to the dataset.
    \item Record replay progress by updating the ZIL header to an opaque value provided by the replay algorithm.
\end{enumerate}
Recording replay progress has two advantages.
First, it makes replay crash consistent. If the system crashes during replay, and replay is retried after reboot, the replay algorithm skips entries that have already been applied based on the progress information.
...
We visualize the logical log structure that the replay algorithm operates on as a two-dimensional grid.
Entries are placed in the grid according to the their generation (x-axis) and transaction group (y-axis).
TOOD

Specifying dependencies through generations results in a list-like structure.
When starting a new generation for every entry, the structure is a chain of sequentially dependent entries.
Figure~\ref{fig:prb_logical_structure:seqchain} shows an example for such a structure.
When writing multiple entries without starting a new generation, the result is still a chain albeit with multiple entries in each segment.
When visualized as a dependency graph, the generations with multiple entries have a diamond shape.
Hence we refer to such log structures as \textit{diamond chains}.
We provide an example in Figure~\ref{fig:prb_logical_structure:diamondchain}.

Note that the purpose of diamond chains is to allow multilpe threads to write log entries to the same log in parallel.
If the storage hardware can maintain low latency, writing the entries in parallel improves the system's overall scalability.
We present an application of diamond chains to ZVOLs in Section~\ref{sec:itxgbypass} and show their performance benefits in our evaluation in Section~\ref{ch:eval}\todo{precise}.

\begin{figure}[h]
    \begin{subfigure}{\textwidth}
        \includegraphics[height=5cm]{fig/prb_logical_structure__seqchain}
        \caption{Each entry was written to a new generation.}
        \label{fig:prb_logical_structure:seqchain}
    \end{subfigure}
    \begin{subfigure}{\textwidth}
        \includegraphics[height=5cm]{fig/prb_logical_structure__diamondchain}
        \caption{Entry \#3 was written without starting a new generation.}
        \label{fig:prb_logical_structure:diamondchain}
    \end{subfigure}
    \caption{An example for the logical structure of a single HDL's log. Both variants contain the same entries but the dependency structure differs due to different generation assignments.}
...
During replay, PRB guarantees that an entry $E$ is only replayed if all of the changes encoded in the entries that $E$ logically depends on have already been applied to $D$.

Such logical 
When an entry is written, the writer provides entry's transaction group $C_{txg}$ and indicate whether the entry should be placed in a new generation.
During replay, this information is sufficient to answer the question whether all changes that an entry an entry's dependencies change  has been applied.


An entry $E$ logically depends on 


If $E$ depends on a change in another entry $E'$ that is missing and younger than \textit{precrash-txg}, replay fails with an error that identifies $E$ as the \textit{offending entry}.\textit{language?}

This information in addition to the \textit{precrash-txg} is sufficent to determine whether an entry's can be replayed.

The simplest log structure that  is a chain of entries where each entry depends on its predecessor.
To produce such a structure the log writer starts a new generation every time it writes a log entry.
Figure~\ref{fig:prb_logical_structure:seqchain} visualizes such a sequential chain.

An entry that does not logically depend on the other entries in the open generation can be written without starting a new generation.
When visualizing the resulting log as a dependency graph, generations with multiple entries have the shape of a diamond.
Hence we refer to such log structures as \textit{diamond chains}.
We provide an example in Figure~\ref{fig:prb_logical_structure:diamondchain}.


If an entry can not be replayed, replay fails with an error that identifies the entry whose dependencies are missing as the \textit{offending entry}.


needs to  should 
The only metadata that PRB requires when writing an entry is is the transaction group $C_{txg}$ in which the logical its logical change was made.
 and an indication whether a new \textit{generation} needs to be started for the entry.
Generations are PRB's concept to express logical dependencies between entries.
Dependencies are critical for replay correctness.

only entries whose dependencies are either alreadhave been replayed and that an entry will not be replayed if its  it depends on one or more missing entries.




The replay algorithm takes as inputs a callback and the last transaction group that was successfully synced to the zpool before the system crashed (:=\~\textit{precrash-txg}).
The callback is invoked for exactly those log entries that have been successfully committed to PRB but not synced to the zpool before the system crashed.
It is the callback's responsibility to atomically apply the change encoded in the log entry and to record replay progress in the zpool.





Generations are the concept to express logical dependencies between entries.
If an entry is missing during 
Entries within the same generation do not depend on each other.
But an entry from a later generation depends on all log entries in all previous generations.
The replay algorithm respects these dependencies when determining the replay order and refuses to replay an entry if one or more of its dependencies is missing, e.g., due to data corruption.

PRB takes care of crash consistency, data integrity, scalability on the write path, and correct replay on the recovery path.


 log that PRB consumers write through a HDL is that of a \textbf{sequence of generations of log entries}.
\begin{description}
    \item[Log Entry] A log entry describes a change $C$ to the zpool state that will be synced to disk in a certain transaction group $C_{txg}$
        The consumer of PRB encodes $C$ as an opaque blob $LR(C)$ and writes it to the PRB using a method on its HDL.
        The only metadata required is $C_{txg}$ and an indication whether the entry should start a new generation or whether it should be added to the currently open generation.
    \item[Generation] Generations are used to express logical dependencies between entries.
        Entries within the same generation do not depend on each other.
        But an entry from a later generation depends on all log entries in all previous generations.
        The replay algorithm respects these dependencies when determining the replay order and refuses to replay an entry if one or more of its dependencies is missing, e.g., due to data corruption.
    \item[Sequence] The log is a sequence of generations.
        Entries can only be added to the most recent generation.
        It is referred to as the \textit{open generation}.
\end{description}
The simplest log structure is a chain of entries where each entry depends on its predecessor.
To produce such a structure the log writer starts a new generation every time it writes a log entry.
Figure~\ref{fig:prb_logical_structure:seqchain} visualizes such a sequential chain.
...
The purpose of diamond chains is to allow multilpe threads to write log entries to the same log in parallel.
For example, if a file is written simultaneously by two different threads, the write operations cannot be logically interdependent.
Consequently, the log entries for these writes can be placed into the same generation.
If the storage hardware can maintain low latency, writing the entries in parallel improves the system's scalability.

After a system crash the log chain can be replayed.
The replay algorithm takes as inputs a callback and the last transaction group that was successfully synced to the zpool before the system crashed (:=\~\textit{precrash-txg}).
The callback is invoked for exactly those log entries that have been successfully committed to PRB but not synced to the zpool before the system crashed.
The invocations happen sequentially in a deterministic order that respects the logical dependencies expressed through the generations.
It is the callback's responsibility to atomically apply the change encoded in the log entry and to record replay progress in the zpool.

PRB does not require log entries to be idempotent % thus the "exactly" in the previous paragraph.
which has two critical implications for replay:
First, replay progress must be recorded atomically with the application of the change.
We have already described above how we delegate this responsibility to the callback.
Second, PRB must ensure that only entries from txgs younger than the precrash-txg are replayed since entries for txgs older or in in the precrash-txg are already part of the synced out state.
We solve this problem by filtering out entries that were written for txgs before or equal to the precrash-txg, using the metadata provided by the log writer.
...
PRB exposes an API that implements the conceptual model presented in the previous section.
...
It is the API user's responsibility to ensure that the PRB is initialized with the same chunks of PMEM for its entire lifecycle.
...
We elaborate on this topic in a later section.
...
We instantiate HDLs using the \lstinline{zilpmem_prb_setup_objset} API on pool import or whenever a new head dataset is created.
The returned HDL is owned by the caller and can be used to create a log, and write, replay or destroy it.
When the head dataset is destroyed or the pool is exported, we must call \lstinline{zilpmem_prb_teardown_objset} to free the HDL in-DRAM state.
...
For modularity and testability, we split the responsibility of persisting HDL state: whereas the HDL APIs are responsible for serialization, the API user is responsible for persistence.
...
When the pool is imported after a crash we must \textit{claim} log entries to prevent them from getting overwritten until they have been replayed.
...
Claiming can fail, e.g., if entries are lost due to data corruption but depended upon by other entries.
The caller defines the error handling policy.
...
After we have completed claiming we can replay individual logs using \lstinline{zilpmem_prb_replay}.
It invokes the \lstinline{zilpmem_replay_cb_t} callback for each log entry that needs to be replayed.
The callback is responsible for loading the log entry's body from PMEM and for executing a DMU transaction in which the log entry's effect is applied and the ZIL header updated to the value of \lstinline{upd}.
The callback does not access PMEM directly but uses the \lstinline{zilpmem_prb_replay_read_replay_node} function instead.
The function handles machine-check exceptions (MCEs) and validates the PRB-managed body checksum.

\lstinline{zilpmem_prb_replay} can fail either due to an error in the callback or due to missing log entries.
The API user can examine the log entry that prompted the failure and decide whether to retry or give up\todo{retry with an override flag that discards the unreplayable part of the log}.
End of replay must be acknowledged explicitly by calling \lstinline{zilpmem_prb_replay_done}.
It is also possible to destroy the log using \lstinline{zilpmem_prb_destroy_log}.
...
The \lstinline{zilpmem_prb_create_log_if_not_exists} performs this task idempotently.
Its return value indicates whether a header update is actually necessary.

Once the log has been created we write entries to it using \lstinline{zilpmem_prb_write_entry}.
...
\begin{description}[noitemsep]
    ...
    \item[needs\_new\_gen] Indicates whether a new generation should be started for this log entry (see Section~\ref{di:prb:structure}).
        The caller of \lstinline{zilpmem_prb_write_entry} is responsible for serializing the start of a new generation:
        if a thread invokes \lstinline{zilpmem_prb_write_entry} with \lstinline{needs_new_gen == true}, that thread must be the only thread executing that function for the given HDL.
        The inverse\todo{contrary?} is not true: if \lstinline{needs_new_gen == false}, multiple threads may execute \lstinline{zilpmem_prb_write_entry} in parallel for the same HDL.
\end{description}
...
Whenever \textit{txg sync} has finished syncing a txg to the main pool it must call \lstinline{zilpmem_prb_gc} with \lstinline{synced_txg == the synced txg}.
...
The PRB's persistent data structure encodes the state of all HDLs as well as the logical structure and content of each HDL's logs.
...
The persistent data structure is comprised of three entities:
...
\end{description}
In the remainder of this subsection we describe the structure of these entities and how they relate to each other.
...
As explained in Section~\ref{di:prb:api:setup} the responsibility to partition PMEM into segments, allocate \lstinline{prb_chunk_t} DRAM objects and add them to the PRB during pool import is delegated to the PRB consumer.
Therefore, PRB does not need to define persistent structures that track PMEM partitioning.
From PRB's perspective, chunks are mere slices of PMEM defined by a kernel-virtual \textbf{base address} and a \textbf{length}.
The chunks' base address is allowed to change every time the PRB is instantiated as long as the effective PMEM space that it refers to remains the same.
...
Chunks act as containers for entries.
...
Then, the next entry starts at this 256 byte multiple.
...
Note that chunks must be sufficiently large to hold at least one entry because entries are never split across chunks.
...
\subsubsection{Entry}
Log entries are represented as a fixed-length header and a variable-length opaque body, as visualized in Figure~\ref{fig:prb_physical_data_structure__chunklayout}.
...
\begin{itemize}[noitemsep]
    ...
    \item attribute entries to a given HDL, and to subsequently
    \item determine a replay order for each HDL's log that respects the logical structure defined by the log writer (see Section~\ref{di:prb:structure}).
\end{itemize}
...
We attribute entries to a given HDL's log through the \textit{log GUID}:
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
...
Claiming and replay use the following data to determine a replay order:
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    \item[Transaction Group (txg)] The transaction group in which the entry's effect was or would have been synced out by \textit{txg sync}.
    ...
    \item[Generation Number (gen)] We represent generations as unsigned 64-bit non-zero integers.
        The ordering of generations in the logical structure must be reflected in the generation number's value:
        when a new generation is started its generation number must be greater than any previous generation number.
    ...
    \item[Generation-Scoped ID (gsid)] Within a generation, we identify each entry by a unique unsigned 64-bit non-zero integer called \textit{generation-scoped-id} (GSID).
        The GSID is only scoped to the generation. Thus, different generations may re-use GSIDs.
    \item[Counters] The number of entries that have been written up to generation $gen$, by transaction group.
        Only the counters for transaction groups $txg$, $txg - 1$, and $txg - 2$ are stored.
\end{description}
We describe the algorithm for claiming and replay in Section~\ref{di:prb:recovery}.


\subsubsection{ZIL header}
The ZIL header (\lstinline{zil_header_pmem_t}) is the persistent representation of HDL state.
However, not every aspect of the runtime state of HDL is reflected in the ZIL header, but only the information needed for log recovery.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    ...
    \item[logging] The log has been created and entries might have been written but claiming has never been completed.
        The log remains in this state for the entire time in which the log is written. Associated data: \textit{Log GUID}.
    \item[replaying] The log has been claimed and replay is ongoing. Associated data: \textit{Log GUID}, \textit{serialized replay algorithm state}.
\end{description}
