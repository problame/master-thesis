...
In Section~\ref{sec:pmemspavdev} we describe how we make the allocatable space of a PMEM SLOG available for direct access by ZIL-PMEM.
...
With this preparatory work out of the way, Section~\ref{sec:zilpmem:zilog} then presents the \lstinline{zilog_pmem_t} object with implements the ZIL-PMEM ZIL kind.
...
\begin{itemize}[noitemsep]
    ...
    \item The ability to access the PMEM SLOG vdev's allocatable space via the PMEM programming model is vital for its use by PRB/HDL.
        If the ability is lost, the PMEM SLOG vdev must be considered faulty and pool import must be refused.
    \item The allocatable space of the PMEM SLOG vdev must be reserved for use by ZIL-PMEM.
\end{itemize}
Our approach consists of two steps.
The first step makes the VDEV layer generally aware of persistent memory devices.
The second step then presents how ZIL-PMEM reserves the allocatable space on PMEM SLOGs.
...
Note that reads and writes to VDEV labels which store parts of the zpool config are located outside of the allocatable space.
...
The pool import procedure first instantiates the VDEV' DRAM representation and registers them with the metaslab alloctor.
After the vdevs structures have been allocated, we add search for PMEM SLOG vdevs.
If the pool contains exactly one PMEM SLOG vdev in the \textit{exempt} class, the pool's ZIL kind is ZIL-PMEM and we allocate the PRB structure.
...
If the single PMEM SLOG vdev in the \textit{exempt} class cannot be found, the pool's ZIL kind is ZIL-LWB and we neither allocate nor free the PRB.

Immediately after allocating the PRB object, we allocate the chunk objects and add them to the PRB.
...
Due to time constraints, we use a simplistic partitioning scheme that does not need to persist any metadata:
\begin{itemize}[noitemsep]
    \item Chunks have a hard-coded chunk size of \SI{128}{MiB}.
    \item We partition the allocatable space as a contiguous array of PMEM segments of that size, starting at offset zero.
    \item Assuming \lstinline{A}~bytes of space, this yields \lstinline{nchunks := A >> 27} chunks and leaves \lstinline{nchunks * (1<<27)} bytes of wasted space.
    \item We do not store \lstinline{nchunks} anywhere. Instead, we prohibit resizing of PMEM SLOG vdev which allows us to deterministically re-compute the value.
\end{itemize}
...
In contrast, the per-dataset runtime state is allocated on demand (\lstinline{dsl_dataset_t}, its \lstinline{objset_t} and the objset's \lstinline{zilog_t}).
Demand allocation means that a consumer of the DRAM structure \textit{holds} it by ID.
If the consumer is the first holder, the \textit{holding} procedure allocates the runtime object and recovers the state from the corresponding persistent structure (\lstinline{dsl_dataset_phys_t}, \lstinline{objset_phys_t} and, partially, \lstinline{zil_header_t}).
It then increments a reference counter and returns the pointer to the consumer.
Conversely, if there was already another holder, the allocation and recovery step are skipped.
...
The mismatch of HDL and dataset object lifetimes is relevant to ZIL-PMEM because the methods of \lstinline{zilog_pmem_t} that implement ZIL-PMEM need access to their dataset's HDL for claiming, replay, and writing log entries.
...
Whenever the ZIL-PMEM implementation needs access to th dataset's HDL, it looks up the HDL in the search tree and aquires a reference to it.
...
Its role is that of an adaptor between the ZIL kind independent \textit{itxg} structure and the HDL that is associated with the dataset via the \textit{HDL map}.
...
\end{enumerate}
To avoid reference counting overhead for HDLs on the write path, we hold a HDL reference once when the ZIL is opened for writing and store it in the \lstinline{zilog_pmem_t} until the ZIL is destroyed because the dataset is unmounted.

The following cases required additional adapter code:
\begin{description}
    \item{Error Handling During Pool Import \& Claiming}
        The vtable was extracted from the original ZIL(-LWB) and inherits its error handling model:
        ZIL-LWB checks the ZIL chain twice during pool import.
        ...
        (Note that \lstinline{zil_claim} only has a return value due to software-technical reasons. It must always return zero.)
        ...
        In contrast, PRB/HDL does checking and claiming in a single pass (\lstinline{prb_claim}), reports corrupted log state through a return value, and is designed to correctly handle online log corruption during claiming and replay (see Section~\ref{di:prb:ccrecovery}).
    ...
    \item[ZIL Header Updates]
        ...
        Unlike most components at this software layer of ZFS, the ZIL implementation is responsible for queuing up changes that need to applied for the \textit{open}, \textit{quiescing} and \textit{syncing} txg.

        For ZIL-PMEM, header updates are simple because HDL always provides a full, updated version of the ZIL header as a \textit{value}.
        We queue the updates using the common ZFS idiom of a 4-ary array that is indexed by \lstinline{[txg&3]}.
    ...
    \item[WR\_NEED\_COPY Chunking]
        Remember from Section~\ref{wrneedcopy} that the ZIL queues up ITXs for \textit{all} changes to a dataset in DRAM.
        To avoid unnecessary memory usage and performance overhead, ITXs that log write operations are not always prepared as a ready-to-write copy of the log entry.
        If a write is not likely to \lstinline{zil_commit}ted or exceeds a certain size, the \lstinline{zfs_log_write} function creates a \lstinline{WR_NEED_COPY} ITX.
        Such ITXs only contain the object number of the DMU object and the range that was modified.
        The creation of the log record is deferred until \lstinline{zil_commit} which splits up the range into segments that fit a \lstinline{WR_COPY} log entry.
        The effect is that the single \lstinline{WR_NEED_COPY} ITX is written as a series of \lstinline{WR_COPIED} records.

        For ZIL-PMEM, we implement a re-usable abstraction that turns a single \lstinline{WR_NEED_COPY} record into an iterator over \lstinline{WR_COPY} entries.
        For each entry yielded by the iterator, \lstinline{zil_commit} writes \lstinline{prb_write_entry}.
        Whereas the repeated chunk aquisition and dependency tracking incurs a small overhead, this approach increases fairness among HDLs in the case of committer slot contention.
