...
\subsection{PMEM Space Reservation}\label{sec:zilpmemzilkind:spacereservation}
...
For this purpose, we introduce a new allocation class called \lstinline{exempt} that is, by convention, never used for allocation.\todo{maybe always allocate all of it so we can get rid of all the special cases?}
...
\begin{itemize}[noitemsep]
    \item Chunks have a hard-coded chunk size of 128~MiB.
    \item We partition the allocatable space as a contiguous array of PMEM segments of 128~MiB, starting at offset zero.
...
\subsubsection{OpenZFS Background: ZVOLs And The ZIL}\label{sec:itxgbypass:openzfsbackground_zvols}
