...
\begin{description}
    ...
    \item[Ordering] Due to pipelined, our-of-order, or speculative execution\todo{forgot sth?}, the program order of stores does not necessarily correspond to the order observed from outside the thread that issues them.
        This is a well-known problem in the context of synchronization between CPU cores, but it applies to Persistent Memory as well.
        To guarantee that stores reach PMEM in a certain order, it is necessary to insert a store barrier instruction (\lstinline{SFENCE}) between them.
        ...
        With regard to ZIL-PMEM, we cover this and the previous two\todo{check number} aspects of PMEM programming in Section~\ref{di:prb:write:crashconsistency} where we describe how ZIL-PMEM stores log records in PMEM.
        \cite{rudoffPersistentMemoryProgramming2017}
...
It is not used for any new DMU transactions, but some DMU transactions assigned to it may not yet have completed.\todo{may not have yet?}
...
As explained in Section~\ref{ch:openzfs_background:zillwb_persistence}, ZIL-LWB uses the SPA to allocate log-write blocks (LWBs) from the storage pool with a bias towards SLOG devices.
In contrast, ZIL-PMEM disables exempts the PMEM SLOG device from the SPA and uses the PMEM space directly.

We partition the PMEM SLOG's space into fixed-size contiguous segments called \textit{chunks}.
We develop a data structure called \textit{PRB} that consumes these chunks and exposes the abstraction of an unordered persistent storage layer for \textit{log entries}.
A log entry is the unit of data that can be written to and read from PRB.
It consists of the ZIL's log \textit{record} and PRB-specific metadata.
...
It is a thin wrapper around HDL's methods for writing and replaying log entries.
\lstinline{zilog_pmem_t} is implemented in the \textit{zil\_pmem.c} code module.
This module also contains the code that sets up PRB/HDL on top of the PMEM SLOG vdev, and the code that synchronizes the lifecycles of HDLs their corresponding datasets.
...
Section~\ref{di:prb:gc} then explains how garbage collection removes entries from it\todo{help, don't want to repeat 'in-PMEM structure'}.
...
For that purpose, a VFS operation summarizes each logical change $C_i$ that it makes in each DMU transaction $T_i$ as an \textit{intent log transaction} (ITX).
...
\item If $T > R_{I_T}.txg$, we can infer that txg $R_{I_T}.txg$ must have been synced out because there are only three unsynced txgs at any given time but four array entries that are reused circularly, curtesy of indexing by $T \bmod 4$.
...
ZIL-PMEM does not currently support WR\_INDIRECT records due to this problem as well as another WR\_INDIRECT-related issue that also affects ZIL-LWB \cite{OpenZFSGithubIssueZilLeaksClaimedBlocksInSeveralEdgeCases}.
...
\begin{description}
    ...
    \item[Header: Detected Data Corruption]
        If the header checksum validation fails, the header was either corrupted by bitflips or similar phenomena or never completely written.
...
This is sufficient to prevent any farther traversal of the chunk.
...
For this purpose, a thread that wants to write an entry \textit{acquires a commit slot} $S \in {0, 1, \dots, ncommitters-1}$.
...
A thread that acquires a commit slot is allows to write its entry to the slot's \textit{open chunk}.
...
\begin{description}
    ...
    \item[Cache Efficiency] Our acquisition procedure deterministically picks the lowest available commit slot.
        ...
        We briefly experimented with per-core commit slots during development and found no noticeable performance difference to the approach presented above.
\end{description}
...
Compatibility code for versions of ZFS prior to the \textit{fast snapshots} rely on ZIL suspend \& resume for taking snapshots.
...
For example, losing a claimed-but-not-replayed log entry can leak space in the main pool or cause a crash during pool import \cite{OpenZFSGithubIssueHandlingOfLostClaimedNotReplayedLogRecords,OpenZFSGithubIssueFailingDebugAssertionWhenLosingClaimedNotReplayedLWBs}.
...
If a block device consumer actually requires consistency at a given point in time, it already \underline{explicitly} indicates this to the block layer.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    ...
    \item[Independence of zil\_pmem.c]
        ...
        It also simplifies the testing code because there no need to mock the environment that zil\_pmem.c expects.
