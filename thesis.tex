...
Write-back is controlled by relative thresholds on the cache device space utilization (\textit{low} and \textit{high watermark}, default to 45\% and 50\%).
...
We use this system for all benchmarks except for Section~\ref{sec:ncommitters_scalability} where we evaluate the \textit{commit slot} abstraction on a system with higher core count and more Optane DIMMs.
