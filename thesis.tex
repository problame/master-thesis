...
\end{itemize}
we come to the conclusion that the current ZIL design is fundamentally unfit to reap the performance advantages of PMEM.
...
The motivation for this thesis is the significant latency overhead of the current ZIL implementation (ZIL-LWB) compared to the raw PMEM hardware.
In this chapter, we describe the benchmarking setup (Section~\ref{ch:lwb_analysis:setup}) to measure ZIL-LWB's overhead and present the resulting data in Section~\ref{ch:lwb_analysis:results}.
We proceed with a breakdown of the latency overhead by the ZFS components that involved in the ZIL write path.
Our findings (Section~\ref{ch:lwb_analysis:breakdown}) show that up to XX\% of the overall latency for a single 4k synchronous random write operation are ZIL-LWB-specific overhead, including up to YY\% ZIO pipeline overhead.
...
\subsection{OpenZFS Background: ZIL-LWB}
