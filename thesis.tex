...
Our findings (Section~\ref{ch:lwb_analysis:breakdown}) show that approximately 80\% of overall latency are spent on ZIL-LWB-specific code, of which in turn approximately 60\% are spent waiting for the ZIO pipeline to persist the LWBs to PMEM.
We conclude that ZIL-LWBs data structure and the ZIO pipeline are unfit to take advantage of PMEM-level performance, motivating the development of ZIL-PMEM.
...
\begin{itemize}
    \item SMT is enabled.
    \item The entire second CPU socket is disabled in software using the \lstinline{isolcpus=8,9,10,11,12,13,14,15,24,25,26,27,28,29,30,31} kernel command line parameter.
...
Each of the \textit{numjobs} threads performs random synchronous write system calls to a separate file per thread (\lstinline{ioengine=sync}, \lstinline{sync=1}, \lstinline{direct=0}, \lstinline{fsync=0}).
Each file has a size of \lstinline{size=100MiB} which means that the written data volume grows with rising \textit{numjobs}.
...
We measure for \SI{60}{seconds} (\lstinline{time_based=1}, \lstinline{runtime=60}), with a ramp-up time of \SI{2}{seconds} (\lstinline{ramp_time=2}), and set \lstinline{end_fsync=1}.
...
In contrast, without the ZIL, ZFS starts with 200k IOPS with one thread and achieves over 900k IOPS at seven threads.
This exceeds the performance of the PMEM hardware (mostly around 500k IOPS) and demonstrates that the ZIL is clearly the bottleneck in the \textit{zil-lwb} configuration.
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[height=5cm]{fig/evaluation/motivating_fio_benchmark__latency_zoomed.png}
%     \caption{Zoomed section of the latency curves of ZFS in async mode, devdax, and fsdax.}
% \end{figure}
...
\subsection{OpenZFS Background: How ZIL-LWB Works}
foo\todo{figure out what we need to explain the breakdown model and rant on zil-lwb below}

\subsubsection{Analysis}

Given the background knowledge on how ZIL-LWB works, we are now ready to analyze its latency.
...
\begin{figure}[H]
    \centering
    \includegraphics[height=14cm]{fig/evaluation/zillwb_latency_analysis__relative_and_absolute_latency_breakdown}
...
\begin{itemize}
    ...
    \item The LWB timeout mechanism, whose purpose is to improve space utilization in the LWBs, amounts to 4\% of overall latency.
    ...
    \item 45--50\% of overall latency is spent waiting for the ZIO pipeline to persist the LWBs.
        ...
        However, even with this increased data volume per IOP, the results from the previous experiment suggest that with a latency of \SI{3}{us} per 4k write to the raw PMEM hardware, we should expect less than \SI{9}{us} of PMEM write time per LWB.
...
The pipeline-oriented, parallelized architecture of ZIO is important for throughput and provides great flexibility%
\footnote{Many of ZFS’s distinguishing features are implemented in ZIO, including physical block allocation, transparent data and metadata checksumming, compression, deduplication and encryption as well as redundancy mechanisms such as raidz.}%
, but increases latency through context switches\cite{openzfsZILPerformanceImprovements2020}.
\textbf{Second, the persistent representation of the ZIL as a chain of LWBs poses a severe and unnecessary overhead on PMEM.}
For one, the latency amortization provided by LWBs and the timeout mechanism is unnecessary at PMEM-level latencies where it is cheaper to persist the entry directly.
And for another, since PMEM is byte-addressable, the persistent representation is no longer constrainted by block-sized, opening up the possibility of greater space efficiency.
...
It must be handled \textit{correctly} (as outlined in the previous paragraph) and \textit{gracefully} with the following behavior as the baseline:
...
With PMEM, the I/O stack is the CPU microarchitecture and the back-pressure manifests as stalled instructions.
However, from the OS thread scheduler's perspective, threads that stall because they wait for PMEM are indistinguishable from actually busy threads.
...
Since ZIL-PMEM shares PMEM among all datasets in a zpool, we expect that bandwidth exhaustion will happen in practice.
...
For ZIL-PMEM, we partition the PMEM SLOG's space into fixed-size contiguous segments called \textit{chunks}.
...
\section{PRB: Garbage Collection}
