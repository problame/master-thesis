...
ZIL-PMEM maintains the same crash consistency guarantees as the old ZIL towards userspace and use the same checksum to ensure data integrity of the ZIL content.
We validate our core data structure through extensive unit testing and exercise our integration into ZFS using the upstream test suite and stress testing tools.
Our performance evaluation shows a speedup of up to 8x on the same Intel Optane PMEM hardware.
...
We have shown that the current implementation of the ZFS Intent Log (ZIL) is unable to leverage the low latency and high throughput of Persistent Memory (PMEM) when used as a \textit{separate log device} (SLOG).
Our analysis shows that the ZIL's on-disk structure (LWBs) as well as the mechanism to persist it (ZIO pipeline) are the main contributors of latency overhead.
This observation motivated the development of ZIL-PMEM, our new ZIL implementation that exclusively targets PMEM.

Our main contribution is the PRB/HDL data structure which consumes the space of a PMEM SLOG and exposes the abstraction of per-dataset virtual, persistent, and garbage logs (HDLs).
...
Our data structure design allows for efficient, crash-consistent insertion of log entries, ensures data integrity through checksums, and can represent logical dependencies between log entries.
The recovery procedure is crash-consistent as well, handles lost entries correctly and gracefully, and handles \textit{machine check exceptions} (MCEs) via \lstinline{memcpy_mcsafe}.

We integrate PRB/HDL into ZFS such that the old ZIL implementation (ZIL-LWB) and ZIL-PMEM can coexist at runtime.
Our design approach for coexistence (\textit{ZIL kinds}) shares code and data structures that define ZFS's crash consistency guarantees towards userspace, but allows for sufficient flexibility in the persistent representation while minimizing code duplication and thus (hopefully) maintenance cost.
...
We achieve X\% code coverage for our userspace tests, and Y\% code coverage for the kernel module whose code coverage is measured by the ZFS test suite.\todo{code coverage results}
Regarding performance, we were able to demonstrate high speedups over ZIL-LWB in micro- and application benchmarks.
The best microbenchmark result is the 8x (4k synchronous random writes), and the best application benchmark result is a speedup of 5.8x in RocksDB.
The speedup for the same workloads on XFS on top of ZVOLs is, with a few exceptions, at least 2x.
In comparison to other block-oriented systems that were adapted to PMEM (Ext4 \& XFS, on PMEM \& dm-writecache), ZFS with ZIL-PMEM performs exceptionally well for small synchronous operations (Redis, RocksDB), presumably due to lower write amplification.
However, in particular in particular on pure PMEM and enabled DAX mode, the performance advantage of the Linux filesystems is still significant.
Our experimental ITXG bypass with which we sought to demonstrate the benefits of a ``less'' sequential log structure for ZVOLs showed some benefit in the microbenchmark, but not in the application benchmarks.
Our \textit{commit slot} mechanism to avoid excessive on-CPU waiting was shown to be effective for a single non-interleaved Optane DIMM but not necessary with four interleaved DIMMs (all on a single CPU socket).

\todo{refer to literature review}

\section{Future Work}\label{sec:futurework}

We would like to systematically \textbf{verify the speculation on write amplification} that we made in the performance evaluation.
For this purpose, we would need to instrument all storage stacks used in the evaluation.
...
Histograms for the distribution of the size could also be useful to determine the degree to which merging of IO operations takes place.
Whereas eBPF could be used for this purpose, the measurement overhead at the level of PMEM access is not neglibile.
Statically compiled, low overhead, per-CPU counters are likely preferrable.

ZIL-PMEM's \textbf{multi-core scalability if the dataset is shared between threads} is significantly worse than with dedicated datasets.
...
Whereas single-threaded IOPS are the same, \textit{fio-growing} only achieves approximately 50\% of the IOPS at scaling factors (=numjobs) 4 and 8.
For \lstinline{numjobs=4}, we believe that this difference is due to the lack of parallelism when writing to the HDL (Section~\ref{sec:zilpmem:zilog}).
This effect does not grow further for \lstinline{numjobs=8} because we know that we are already limited by PMEM bandwidth at \lstinline{numjobs=4}.
Whereas we are fairly confident about these assumptions, a dedicated experiment to confirm them would be valuable.
...
The important metrics for recovery are a) pool import time, b) replay time, and c) DRAM usage by the HDL's holds on chunks.

Also, the \textbf{influence of chunk size} has not yet been evaluated.
We chose a large chunk size (128 MiB) for our evaluation to avoid contention on the PRB-wide mutex that must be held to aquire new chunks.
% For example, the 128 MiB chunk size allows for $\frac{128 * 2^20}{256 + 192 + 4*2^10 + (256 - 192)} \sim 30840$ 4k log entries per chunk.
% Consequently, only every ${30840^\text{th}}$ log entry would need to aquire the PRB-global mutex to get a new chunk.
Since aquisition of new chunks has not been a relevant contributor to latency in any of our experiments, such a large chunk size is probably not required in practice.
Further, it increases the number of lost log entries if an entry header in the chunk is corrupted.

We explicitly ignored \textbf{space efficiency} as a concern for ZIL-PMEM because of the high capacity of Intel Optane PMEM.
However, determining and potentially reducing the per-entry space overhead of ZIL-PMEM could be useful to reduce reduce media wear-out.
...
We would like to extend our performance evaluation with the following storage stacks:
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    ...
    The prototype avoids unnecessary sequential ordering of I/O operations when writing LWB.
    ...
    \item[Ext4 journal\_dev / XFS logdev] Both XFS and Ext4 provide an option to configure dedicated journaling / write-ahead logging devices.
        The comparison with ZIL-PMEM is thus naturally interesting.
        However, performance during manual experimentation with this feature was unexpectedly low for both filesystems.
        Since both configurations seem rather exotic, we used the configurations presented in the configuration instead.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    \item[PMEM Programming Model Checkers] In Section~\ref{sec:rel_work:pmemspecifictools}, we have surveyed tools that automatically check whether the software under test issues the architectually required instructions for persistence.
        Most were either inapplicable to kernel code or not publicly available.
        However, the \textit{PMTest} tool has limited support for kernel code and could be used to validate our implementation.
    ...
    \item[Fuzzing] Fuzzing could potentially be used harden PRB/HDL against unexpected state in PRB.
        This is particularly relevant if support for OpenZFS Native Encryption is added in the future.
        Our software engineering efforts to facilitate testing of PRB/HDL enable fuzzing at the data structure level.
\end{description}
...
The PRB garbage collection procedure presents an opportunity to eagerly detect media errors by traversing a chunk's content before garbage-collecting it.
If the PMEM hardware detects an error, the problem will be surfaced via an MCE and can be reported to the user.
For end-to-end checking, PRB could also store some form of checksum of a chunk's content in the chunk's DRAM representation that could be validated during garbage collection.
Regarding unsafe shutdowns, it would be useful to have integration with the \lstinline{zpool status} command to detect the problem and potentially prevent import of the pool to make the user aware of this ``very rare event''\cite[Chapter 17]{Scargall2020}.
However, we believe that ZIL-PMEM's existing measures to detect data corruption should be able to discover problems on the fly.
