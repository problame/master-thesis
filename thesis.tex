...
\begin{description}[noitemsep]
    ...
    \item[Log Structure Encoding] We must compute the \textit{Counters Table} every time a log write starts a new generation (see Section~\ref{di:prb:persistent:entry}).
        As the name indicates, we must maintain a precise counter of the number of log entries that were written for each txg.
        This presents a multicore scalability concern.
    \item[Chunk Selection] Before writing an entry we must select the chunk into which the entry should be written, which might require coordination with other writers.
...
\begin{description}[noitemsep]
    \item[Checksumming] Use ZFS's optimized implementations of the Fletcher to compute the entry header and body checksum.
    \item[Writing Entries] Use AVX512 non-temporal store instructions for writing log entries to PMEM.
        For the size of log entries written by ZIL-PMEM, this is the recommended technique for maximum bandwidth in \cite{yangEmpiricalGuideBehavior2020}.
    \item[PMEM Bandwidth Limits \& Multicore Scalability] The PMEM programming model dictates that I/O wait time is spent on-CPU --- typically at a memory barrier instruction or because the CPU has exhausted its store or load buffer capacity\todo{review terminology; need proof?}.
        ...
        Given that PRB shares PMEM among all HDLs, and that HDLs can be written in parallel, our design must address this concern.
\end{description}
...
If the thread is starting a new generation, this step involves the computation of the counter table that the entries of the new generation will contain.
The next step is the selection of a the target PMEM chunk that has sufficient space for the entry. This task falls in the domain of the committer slot abstraction.
After the chunk has been aquired we can write the entry which consists of several sub-steps:
...
\begin{figure}[H]
    \centering
    \includegraphics[height=14cm]{fig/prb_writepath_highlevel_activity_diagram}
    \caption{Flow chart of the activities on the write path.}
\end{figure}
...
Each thread that is simultaneously committing has a different committer slot.
If more threads want to aquire a committer slot than are available, the function to aquire the slot blocks.

Associated with each committer slot is the ownership of a PMEM chunk.
While a slot is held by a thread, the thread owns the associated chunk.
The thread can write its entry to the currently associated chunk or replace it with a new one if the capacity of the current chunk is insufficient.
Replacing a chunk involves putting the chunk on the \textit{full queue} where it will be eventually garbage-collected, and dequeuing a new chunk from the \textit{free queue}.
Dequeuing a chunk from the \textit{free queue} might fail if there are no free chunks.
The log writer can choose whether to block and wait or fail the log entry write operation with an error.\todo{no deadlock potential with zil\_commit because it is called without DMU tx open, but that's too much detail here. also the current impl doesn't handle non-block correctly because we don't rollback the dependency tracking state. but actually we need nonblock for itxg bypass so maybe we should always do nonblock}
Regardless of whether the chunk is replaced or not, the ownership of the chunk that was effectively written to moves to the slot as soon as the thread releases its hold on it.
Figure~\ref{fig:flowchart_committer_slot_aquision_and_chunk_selection} visualizes the procedure described above.
...
We use atomic compare-and-swap operation and GCC's \lstinline{__builtin_ffs} function to find and flip a zero bit in the bitmask without requiring a lock.
We retry indefinitely if the compare-and-swap operation detects a race with another thread.
While this technique does not technically guarantee a bounded wait time, we found it to not be problematic in practice in our evaluation.\todo{we have the dtrace probe, but I think we don't have an automated benchmark for this yet.}
