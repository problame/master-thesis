...
Our userspace \textit{tests for PRB/HDL} enabled fast iteration on the implementation with high confidence in its correctness.
...
\textit{Ztest} exposed many implementation errors in the integration code.
Again, the high iteration speed and ease of debugging in userspace proved beneficial.\\
The \textit{ZTS tests} related to the ZIL pass (or are skipped) for both ZIL-LWB and ZIL-PMEM.
...
We compare the line coverage in the ZIL code modules for the userspace tests (\textit{ztest} and PRB/HDL tests) and kernel module tests (ZTS SLOG tests listed above) between our tree of OpenZFS and the merge-base commit of our tree with the upstream \lstinline{master} branch.
For userspace coverage, we executed the PRB/HDL tests once and the \textit{zloop.sh} script for 60 minutes for each ZIL kind.
In the upstream tree, we only executed \textit{zloop.sh} once (60 minutes runtime).
For kernel coverage, we executed the ZTS tests tagged as \textit{slog} (\lstinline{zfs-tests.sh -T slog}) which corresponds to the SLOG tests listed above.
We present the results in Figure~\ref{fig:eval:correctness_line_coverage}.

For our tree, \textit{zloop.sh} encountered 5 crashes (120 min runtime), two of which were in ZIL-PMEM code.
For the upstream tree, it found 1 crash (60~min~runtime).

\newcommand{\cssubheader}[1]{\multicolumn{1}{c}{ \rotatebox[origin=c]{0}{\scriptsize #1} } }
...
we have no evaluation on impact of interrupt masking, future work~\ref{di:prb:write:crashconsistency}
