...
If the subtraction would overflow or if the \textit{live table} does not contain counters for the given txg, value in the \textit{last table} is $(0,0)$.
...
%\end{lstlisting}
%
%When writing an entry whose txg does not match $T_i$ we know $T_i$ has been synced because there are only three unonly three  that When writing an entry $E$ for a transaction group $$ we find the row $R_{T\&3}$ in the table.
%
%We use a  \textit{live} table, we use 
%
