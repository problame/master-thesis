...
ZIL-PMEM is added later in Section\ref{sec:zilpmem} as another ZIL kind.

\subsection{Enabling Multiple Strategies For ZIL Persistence At Runtime}

As described in Section~\ref{openzfs:the_zil_api} the ZIL(-LWB) in upstream ZFS is implemented by the per-dataset object \lstinline{zilog_t}.
...
\lstinline{zil_commit} drains the ITXs into a \textit{commit list}.
The log entries in these ITXs are packed into LWBs which are written to disk as a linked list (\lstinline{zil_chain_t}), the links being ZFS block pointers.
The root of the ZIL chain is stored in the ZIL header along with other fields that track claiming and replay progress.
...
\begin{itemize}
    \item It defines the framework for ZFS's crash consistency semantics.
        Whereas ZPL and ZVOL code are responsible for filling in the contents of the log records, the overall semantics are defined by the code that assembles the commit list.
        (See Figure~\ref{lst:zil_commit} for pseudo-code and Figure~\ref{fig:zil_per_itxg_state_example} for an example).
    \item ITXs and even the commit list are independent of the LWB chain that is ultimately written to disk.
        The commit list merely defines the the order in which the log records should be replayed.
    \item The interface between the ITX-related code and the LWB-related code is the \textit{commit list}.
        Although the ITXs are not opaque to the LWB-related code (e.g. handling of \lstinline{WR_NEED_COPY}\todo{check that this was defined}), the responsibilities are cleanly separated.
\end{itemize}
Given these insights we refactor \textit{zil.c} as follows:
\begin{enumerate}
    ...
    \item Virtualize calls of \lstinline{zillwb_} functions:
        \begin{itemize}
            ...
            \item Replace all calls to \lstinline{zillwb_FN} functions in \textit{zil.c} with indirect calls through calls through the vtable, i.e., \lstinline{zilog->zl_vtable.FN(...)}.
        \end{itemize}
    \item Make non-ITX state private to \textit{zil\_lwb.c} by turning it into a subobject.
        \begin{itemize}
            ...
            \item Add a member \lstinline{zlvt_alloc_size} to \lstinline{zl_vtable_t} that indicates the amount of memory to be allocated when allocating a \lstinline{zilog_t}.
            \item Add constructor and destructor functions to the vtable that are called after allocating the \lstinline{zlvt_alloc_size}d \lstinline{zilog_t}.
                The \lstinline{zillwb_} constructor and destructor cast the \lstinline{zilog_t} pointer to a \lstinline{zilog_lwb_t} pointer, then set up or tear down the private state.
        \end{itemize}
\end{enumerate}
The end result can be best-described in the terminology of object-oriented languages:
\textbf{\lstinline{zilog_t} is an abstract baseclass} that
\begin{itemize}
    \item implements the public ZIL interface,
    \item implements the ITX-related functionality of the ZIL, and
    \item defines abstract methods for persisting log records which must be implemented by concrete subclasses.
\end{itemize}
\textbf{\lstinline{zilog_lwb_t} is a concrete (final) subclass} that implements the current LWB-based persistence strategy.
Figure~\ref{fig:zilog_splitup_class_diagram} visualizes these changes as a comparison of UML class diagrams.
Figure~\ref{fig:zilog_splitup_example_code} gives an example of the refactoring steps described above.

\subsection{}
The refactoring of \lstinline{zilog_t} enables different strategies for ZIL persistence at runtime.



\lstinline{zilog_lwb_t} embeds  persistence-related  \lstinline{zilog_t} chose to extract the 
We observe that the code that handles ITXs and \lstinline{itxgs} is only used on ZIL write path, not during recovery.
It has a 
Claiming and replay only deals with LWBs

Our strategy for coexistence between ZIL-LWB and ZIL-PMEM is to
\begin{itemize}
    \item factor out 


Our strategy for coexistence bewteen 

   become obsolete due to txg sync or must be or a request to persist is issued 
