...
Then, it is available for writing \textit{log entries}.\todo{language... help}
...
Our evaluation in Section~\ref{ch:eval}\todo{precise} demonstrates that significant scalability improvements over baseline ZIL-PMEM with sequential commits lists are possible without weakening crash consistency guarantees.
...
The caller decides whether to retry replay or abandon the unreplayable part of the log\todo{retry with an override flag that discards the unreplayable part of the log}.
...
%     \item[Chunk Ownership: Chunk Lists \& Committer Slots] PRB needs  uses chunks to allow as the  tracks chunk ownership  the chunks that were assigned to it during construction in a set of lists.
%     We describe committer slots below.
% \end{description}
...
Within the PRB, the chunk is either put on one of several lists or it is held by a \textit{committer slot}.
The number of committer slots is determined by the \lstinline{ncommitters} argument\todo{param?} during PRB construction.
A committer slot can hold at most one chunk at any given time.
When a log write wants to write an entry it first aquires a commit slot and then writes the entry to the commit slot's chunk.
...
\begin{description}[noitemsep]
    ...
    \item[Full Lists]
        ...
        The implementation follows the common ZFS idiom of using four lists that are indexed by \lstinline{T_i & 3} where \lstinline{&} is a \textit{bitwise and} operation.
...
\begin{figure}[H]
    ...
    \caption{
        ...
        The thread places chunk 2 on the correct \textit{full list} for chunk 2's \textit{max txg} and finds a new chunk on the free list for the committer slot (not shown).
...
Our primary goal is performance: we want our design to minimize PRB's latency overhead and maintain this for parallel writers on a multicore system.
...
\begin{description}[noitemsep]
    ...
    \item[Log Structure Encoding] We must compute the \textit{Counters Table} every time a log writer starts a new generation (see Section~\ref{di:prb:persistent:entry}).
        As the name indicates, we must maintain a precise counter of the number of log entries that were written for each txg.
        This presents a multicore scalability concern since the counter is shared across all log writers.
    \item[Chunk Selection] Before writing an entry we must select a target chunk. This selection process might require coordination with other writers, depending on the implementation.
...
\begin{description}[noitemsep]
    ...
    \item[PMEM Bandwidth Limits \& Multicore Scalability] The PMEM programming model dictates that I/O wait time is spent on-CPU --- typically at a memory barrier instruction or because the CPU has exhausted its store or load buffer capacity\todo{review terminology; need proof?}.
        This is problematic from a CPU utilization perspective: if multiple threads attempt to write at higher bandwidth than PMEM can sustain, they still appear busy towards the OS thread scheduler and waste CPU time that could be used more productively by other threads in the system.
        \citeauthor{yangEmpiricalGuideBehavior2020} have shown that a single Optane DIMM's write bandwidth can be exhausted by one CPU core at \SI{2}{GB/s}.
        Write bandwidth decreases to \SI{1}{GB/s} at ten or more concurrently writing CPU cores ~\cite{yangEmpiricalGuideBehavior2020}.
        Whereas excessive on-CPU waiting might be the right trade-off in certain userspace applications of PMEM, we cannot make assumptions about the system's overall CPU priorities in a filesystem such as ZFS.
        Given that PRB shares PMEM among all HDLs, and that HDLs can be written in parallel, we expect that PMEM write bandwidth could be exhausted in practice.
        Our design mnust therefore find a way to limit concurrent access to PMEM and shift PMEM wait time off the CPU.
\end{description}
...
A thread that writes an entry first aquires a \textit{committer slot} which is our mechanism to limit concurrent writes to PMEM.
Afterwards, it updates the structure that is used to compute the Counter Table.
If the thread is starting a new generation, this step also involves the computation of the Counter Table.
The next step is the selection of a target PMEM chunk that has sufficient space for the entry. We tie this task to the committer slot mechanism.
After the chunk has been aquired we can write the entry.
This step consits of several sub-steps.
...
\subsubsection{Committer Slots}\label{di:prb:write:chunksel}
The committer slot abstraction is our solution to coordinate access to PMEM among multiple parallel writers.
The goal is to allow up to \lstinline{ncommitters} threads simultaneous access to PMEM.
For this purpose, a thread that wants to write an entry \textit{aquires a committer slot} $S \in {0, 1, \dots, ncommitters-1}$.
Each thread that is simultaneously committing gets a different committer slot.
The function to aquire a committer slot blocks the calling thread until a committer slot is available.

As explained in Section~\ref{prb:di:chunkownership}, a committer slot can own up to one chunk.
A thread that aquires a committer slot assumes temporary ownerhsip of the associated chunk.
...
Dequeuing a chunk from the \textit{free list} might fail if there are no free chunks.
...
If the log writer chooses to block and wait during chunk replacement, it must guarantee that it does prevent \textit{txg sync} from making progress to avoid a pool-wide deadlock.
In practice, this means that the log writer must not hold a DMU transaction open when writing an entry with block-and-wait chunk replacement enabled.
...
This is not true for the ITXG bypass for ZVOLs (Section~\ref{sec:itxgbypass}\todo{chapter?}) where we need to use the non-blocking mode.
\todo{maybe we should change the design to always use non-blocking?}

We implement committer slots using a semaphore and a bitmask.
...
While this technique does not \textit{guarantee} a bounded waiting time, we found it to not be problematic in practice in our evaluation.\todo{we have the dtrace probe, but I think we don't have an automated benchmark for this yet.}
The following listing describes the algorithm for committer slot aquisition and release.
...
\subsubsection{Log Structure Encoding}\label{di:prb:write:logstructureencoding}
\subsubsection{}
