...
Our solution, ZIL-PMEM, is then presented Chapters \ref{ch:designoverview}, \ref{ch:zilkinds}, \ref{ch:spavdev}, \ref{ch:prbhdl}, and \ref{ch:zilpmem}.
Chapter~\ref{ch:designoverview} defines the project goals and provides a high-level overview of the overall design.
Our main contribution, the \textit{PRB/HDL} the data structure, is presented in Chapter~\ref{ch:spavdev}.
The other three\todo{check} chapters present changes that we made to ZFS's architecture to enable alternative ZIL implementations, represent PMEM SLOGs in ZFS, and integrate PRB/HDL as a new ZIL implementation.
...
We introduce PMEM to ZFS in Chapter~\ref{ch:spavdev} where we describe our changes to the VDEV layer to support PMEM SLOG devices.
Our main contribution --- PRB/HDL --- is then presented in Chapter~\ref{ch:prbhdl}.
Chapter~\ref{ch:zilpmem} is the the cornerstone\todo{ok?} where we describe how we combine the work presented in the previous three chapters into the ZIL-PMEM ZIL kind.
We evaluate 
...
\chapter{PMEM-aware SPA \& VDEV layer}\label{ch:spavdev}
...
PRB abstracts shared physical PMEM into virtual logs (HDLs) for each dataset in the pool.
