...
The \textit{zil\_pmem.c} module which we present in Section~\ref{sec:zilpmem} is a mere integration layer between the remainder of ZFS and PRB.

\subsection{Overview} % API Overview?
PRB implements a generic write-ahead log for a zpool that stores opaque log entries in PMEM.
A single PRB instance supports multiple logs with independent lifecycles.
Each log is represented through a HDL object that provides methods for writing and replaying log entries.
The logical structure of an individual log allows for limited parallelism when writing log entries but guarantees a deterministic replay order.
Log entries are opaque to PRB --- the only ZFS-specific metadata per log entry is the txg for which it was logged.

% API
% Fließtext, soll v.a. die ganze Terminologie einführen: Chunks, Entry, Dependency, Gc.
%
% Example Write + GC +  Recovery (from unit tests)
% code + some activity diagram for viz

\subsection{Data Structures}
% in-PMEM
% ZIL header

\subsubsection{Dependency Semantics}
