...
We present PRB in a top-down manner, starting with a black-box description of the logical log structure for a single HDL in Section~\ref{di:prb:structure} followed by a walkthrough of the APIs that are used by ZIL-PMEM (Section~\ref{di:prb:api}).
...
Our conceptual model for the log that is written and replayed through a PRB is a \textbf{sequence of generations of log entries}.
\begin{description}
    ...
    \item[Generation] Generations are the vehicle to express logical dependencies between entries.
...
\end{description}
The simplest log structure is a chaun of entries where each entry depends on its predecessor.
To produce such a structure the log writer starts a new generation every time it writes a log entry.
If an entry is lost, none of the subsequent entries can be replayed because they logically depend on the previous entry.
If a log entry does not depend on the open generation's log entries, the log writer can choose to write it without requesting a new generation.
The entry is then added to the existing generation.
...
Figure~\ref{fig:prb_logical_structure} provides an example for both the fully sequential log and the diamond chain.

\begin{figure}
    \missingfigure{see presentation}
    \caption{The logical structure of a single HDL's log. Both variants contain the same entries. Variant a) was written with a request for a new generation for every entry. Variant b) was written without requesting a new generation for entry 3.}
    \label{fig:prb_logical_structure}
\end{figure}

The \textit{diamond chains} are useful to allow multiple threads to write log entries in parallel.
For example, if a file is written simultaneously by two different threads in non-overlapping regions, the write operations cannot be semantically dependent.
This implies that the log entries for these writes can be in the same generation and that they can be written in parallel.
If the storage hardware can maintain low latency, writing the entries in parallel improves the system's scalability.

Note that today's ZIL can be represented as a fully sequential chain because the commit list assembled by the ITX code\todo{ref} is sequential as well.
However, our analysis in Section~\ref{ch:lwb_analysis}\todo{need to show fsync every non-scalability of zil-lwb vs raw hw there} shows that ZIL-LWB does not scale well in such scenarios.
Further, a prototype presented at the OpenZFS 2020 Developer Summit proposes changes to ZIL-LWB to allow for more parallelism.~\cite{openzfsZILPerformanceImprovements2020}
We interpret these findings as indicators that ZIL-PMEM --- and thus PRB --- should support parallelism on the write path in the form of \textit{diamond chains}.
We present an application of \textit{diamond chains} to ZVOLs in Section~\ref{sec:itxgbypass}.
