...
A single DIMM of the current Intel Optane DC PMEM product line can sustain 550k random 4k write IOPS from a single CPU core which corresponds to a write latency of \SI{1.81}{us}.
However, when configuring the Linux PMEM block device as a SLOG in ZFS, a single thread only achieves 10k IOPS~(\SI{100}{us/IOP}), scaling up to 100k IOPS at 7 threads (\SI{70}{us/IOP} per thread).
...
Once the open LWB is full or a timeout has passed, the open LWB is written (\textit{issued}) to disk (see Section~\ref{ch:lwb_analysis:openzfs_background_zil_lwb} for details).
...
The ZIL's structure and its interaction with txg sync is substantially different from the journaling modes presented in the paper (See Sections~\ref{ch:lwb_analysis:openzfs_background_zil_lwb} and~\ref{di:prb:background}).\todo{review ref}
...
\begin{figure}[H]
    \centering
    \includegraphics{fig/zil_lwb__physical_structure_overview}
...
However, the benchmarks show that multi-thread scalability within a \textit{single} dataset is limited.\todo{validate claim or rewrite}
...
\begin{description}[noitemsep]
    \item[Simple Administration] Our design foresees that activation of ZIL-PMEM will be completely transparent to the administrator, based on VDEV configuration (Section~\ref{sec:zilpmemzilkind:activation}).
        In that case, there would be no user-visible change in ZFS's administrative tools.
        However, due to time constraints, our current implementation does not yet supports changing the ZIL kind of the zpool after creation (Section~\ref{sec:zil_kinds:change}).
...
We use unit tests and integration tests to validate\todo{better verb?} that our implementation.
...
\textit{libzpool} contains the majority of the ZFS kernel module's code --- it is used to by \textit{zdb} (the ZFS debugger) and the \textit{ztest} stress testing tool (next section).
...
\begin{description}[noitemsep]
    \item[Dimensionality] ZIL kinds add a new dimension in the configuration space that has not been anticipated by the designers of ZTS and ztest.
        ...
        However, the testing harnesses for ZTS and ztest do not have the infrastructure for this task.
    ...
    \item[VDEV Management]
        A significant number of tests in ZTS and ztest deals with\todo{language} mutations of the pool's VDEV config, e.g., adding, removing, growing, offlining devices.
        ...
        Thus, many tests that would work correctly in the final implementation currently fail when attempting to change the VDEV config.
\end{description}
...
\begin{description}[noitemsep]
    ...
    \item[VDEV Management] There are several \textit{ztest} routines that randomly add and remove VDEVs, inlcuding SLOG devices.
        We change the function that selects the target VDEV such that it never returns SLOG device if the pool's ZIL kind is ZIL-PMEM.
\end{description}
We have not added any ZIL-PMEM specific assertions or additional PMEM SLOG related test routines to \textit{ztest}.
...
\end{enumerate}
The \lstinline|slog_replay_fs_00{1,2}| tests use ZFS filesystem.
The \lstinline|slog_replay_volume| test exercises the ZVOL code paths by instantiating a platform-native file system on top of the ZVOL (ext4 on Linux, UFS2 on FreeBSD).
We modify ZTS as follows:
\begin{enumerate}[noitemsep]
    \item Add a mechanism to skip tests based on the current value of the the \lstinline{zil_default_kind} module parameter (Section~\ref{sec:zil_kinds:change}).
...
We have addressed all of these edge cases in the design and implementation, and thus all of our PRB/HDL tests pass.

\textit{Ztest} helped us find many implementation errors in the integration code (\textit{zil\_pmem.c}).
Again, the fast\todo{high?} iteration speed and ease of debugging in user space proved beneficial.
We have run the \textit{zloop.sh} script for X\todo{value} minutes for each ZIL kind and did not encounter any crashes.

The \lstinline{ZTS} tests listed above pass (or are marked for skipping) for both ZIL-LWB and ZIL-PMEM.
However, they exposed significantly less implementation issues than \textit{ztest}.
...
We start with the familiar 4k synchronous random write workload that motivated this thesis (ref. Section~\ref{ch:lwb_analysis}):
\begin{itemize}[noitemsep]
    ...
    \item What are the tail latencies experienced during the benchmark? How do these compare to ZIL-LWB?
\end{itemize}
...
\end{itemize}
We also answer the following question apply this benchmark suite to ZVOLs:
...
In summary, we configure the system such that it effectively becomes a single-socket system with sufficient DRAM, two \SI{128}{GiB} Optane DIMMs, and three Micron 960GB NVMe SSDs.
We use this system for all benchmarks except for Section~\ref{sec:eval:ncommitters_scalability} where we evaluate the \textit{commit slot} abstraction on a system with higher core count and more Optane DIMMs.
...
\begin{description}[noitemsep]
    ...
    \item[Hardware Resource Registry]  All hardware resources, e.g., NVMe partitions and PMEM namespaces, are registered in a global registry object by label, e.g., label \lstinline{fsdax} for PMEM \textit{fsdax} namespaces.
    \item[Unified Storage Software Configuration] We abstract the setup and teardown of software storage systems such as ZFS or \textit{dm-writecache} as Python \textit{context manager} objects.
        We use the global registry object to discover the hardware on which the software storage stack should be instantiated, thereby improving re-usability on different hardware configurations.
    ...
    \item[Pinned Software Versions] We either build benchmarks from pinned source code versions (git submodules) or use pinned binary versions.\todo{Still need to do this}
    \item[Result Storage] For each benchmark invocation, we serialize a description of the storage stack, benchmark parameters, and results into JSON objects and store these objects as unique files in the filesystem.
        Results can be grouped by a prefix for post-processing, e.g., files with prefix \lstinline{app_benchmarks__v4} contain the results of the fourth iteration of our application benchmark design.
\end{description}
We also automate post-processing of the results in Python Jupyter notebooks using popular Pandas framework and render plots using Matplotlib and/or Seaborn.\todo{need refs for all this software?}
...
\textit{zil-pmem} follows \textit{async} with a near-constant offset for all percentiles until \lstinline{numjobs=4} where we observe the characteristical ``knee in the curve'' which is expectable given that the system's peak IOPS are first achieved at this value of \lstinline{numjobs}.
In contrast, \textit{zil-lwb} base latency (5th percentile, \lstinline{numjobs=1}) is $\frac{36.6}{6.7} = 5.46$ times higher than \textit{zil-pmem}'s and the higher percentiles show a steeper growth with rising \lstinline{numjobs}.
...
For ZIL-PMEM, the most dominant component is the \textit{async} code, i.e, VFS-level work and modification of in-DRAM DMU objects.
In contrast to ZIL-LWB, the shared ITX code is a noticable component at 10\% contribution to overall latency.
Note that the absolute \textit{async} overhead for ZIL-PMEM is lower than for ZIL-LWB, particularly for low \lstinline{numjobs} values.
We have no satisfying explanation for this behavior --- presumably, ZFS's \textit{async} code has non-linear scaling behavior with regards to latency.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    \item[devpmem] The raw \lstinline{fsdax} namespace block device in devfs (\texttt{/dev/pmem0}).
    \item[dm-writecache]
        ...
        Since most benchmarks consume relatively little space compared to \texttt{/dev/pmem0}'s capacity (40GiB), we configure dm-writecache's \textit{low watermark} to 0\% and \textit{high watermark} to 1\% in order to trigger some write-back during benchmark execution.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    ...
    \item[Redis-SET] Redis is a popular in-memory key value store.
        ...
        The only exception is \textit{zfs-sync-disabled} for which the shortest runtime is \SI{8}{s}.
...
The reason for this phenomenon is most likely lower CPU overhead at scaling factor $1$ (compare \textit{zfs-pmem} with \lstinline{sync=disabled}), and better multi-core scalability at higher scaling factors.
...
We assume that ZIL-PMEM handles this workload more efficiently than the Linux filesystems because the ZIL's data journaling causes less write amplification than the Linux filesystems whose latency is likely dominated by page-granular write-back on every \lstinline{fsync}.
...
However, since \lstinline{sync=disabled} does not perform better than any of the systems that actually write to PMEM, we can also rule out PMEM bandwidth as a possible bottleneck.
...
For higher scaling factors, XFS's speedups diminish and even go below 1 at scaling factor $8$, albeit in DAX-aware mode.
We have no satisfying explanation for this behavior.
Also, the fact that \lstinline{sync=disabled} achieves a lower speedup than XFS can be taken as an indicator that \textit{filebench-varmail} is not bound by PMEM performance, contradicting our speculation in the previous section.
...
whereas the ZIL is able to write small log entries directly to PMEM, dm-writecache hides PMEM behind a virtual block device that is not DAX-capable.
Thus, even if the Linux filesystems supported DAX-aware logical logging, this functionality would not come to fuition on top of \textit{dm-writecache}.
...
For example, XFS on raw PMEM without the \textit{dax} mount option achieves 150k IOPS at scaling factor 1 and 409k IOPS at scaling factor 4 in the \textit{fio-growing}.
...
ZIL-PMEM has similar problems (125k IOPS at factor 1, 217k at factor 4) albeit due to the sequential structure of ZIL chain.
In the next section, we show how a more relaxed log structure such as the ITXG bypass can improve the situation.

\subsubsection{Impact on ZVOL Performance \& ITXG Bypass}
...
\begin{figure}[H]
    ...
    \caption{
        Comparison of XFS on ZVOLs on different zpool configurations with \textit{zvol-pmem-rs\_0-byp\_0-nc\_3} as baseline.
    }
\end{figure}
...
And at $numjobs = 12$, $T_{pmem_{iop}}$ climbs to $11.6, 20.6~\text{us}$ for \lstinline{ncommitters=8} and \lstinline{12} whereas \lstinline{ncommitters=3} is successfully limited to $2700~\text{ns}$ of PMEM write time per IOP.
...
It is $1.660~\text{us}$ for $numjobs = 1$ and grows slightly unevenly towards $2.56~\text{us}$ at $numjobs = 24$.
    ...
    \item Regarding the non-interleaved configuration (single Optane DIMM):
        \begin{itemize}
            ...
            \item Note that we used \lstinline{ncommitters=3} as the configuration for ZIL-PMEM in all of the previous benchmarks.
                ...
                Our rationale is that an in-kernel filesystem such as ZFS should not waste CPU time on PMEM I/O because it is a system-wide OS service whose overall priority in the system is not known ahead of time.
