...
Intel Optane DC Persistent Memory is currently the only broadly available Persistent Memory product on the market.\todo{need proof?}
...
For DAX-capable devices such as the \textit{fsdax} /dev/pmem nodes in devfs, the kernel-internal \lstinline{dax_direct_access} API allows for translation of a range of sectors (\lstinline{sector_t}) to a virtual address range.
...
\begin{description}
    ...
    \item[Ordering] Due to pipelined, our-of-order, or speculative execution\todo{forgot sth?}, the program order of stores does not necessarily correspond to the order observed from outside the thread that issues them.
        ...
        With regards to ZIL-PMEM, we cover this and the previous two\todo{check number} aspects of PMEM programming in Section~\ref{di:prb:write:crashconsistency} where we describe how ZIL-PMEM stores log records in PMEM.
        \cite{rudoffPersistentMemoryProgramming2017}
    ...
    \item[Error Handling] The error handling story for PMEM is largely unsatisfactory.\todo{too harsh?}
        ...
        It communicates such problems and potentially other errors to the CPU which surfaces them to the operating system via \textit{machine check exceptions} (MCEs).
        ...
        In ZIL-PMEM, we only handle read errors using \lstinline{memcpy_mcsafe}, and treat them identically to software checksum errors.
        We address potential software-level approaches for detection of write errors in the Section~\ref{sec:futurework} on future work.
