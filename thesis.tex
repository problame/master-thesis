...
\begin{figure}[H]
    \centering
    \includegraphics[height=6cm]{fig/prb_hdl_log_structure_example}
    \caption{TODO}
    \label{fig:prb_hdl_log_structure_example}
\end{figure}

\section{Recovery}\label{di:prb:recovery}
\subsection{Replay}
Replay must apply the changes that were successfully logged to the HDL but whose DMU transaction did not sync before the system crashed ($E.txg > precrash\_txg$).
The \textit{replay order}, i.e., the order in which these changes need to be applied, is given by $(gen, gsid)$:
\begin{displayquote}
Given log entries $E_a, E_b$ of the same HDL log (their $E_a.logid = E_b.logid$), $E_a < E_b \equiv (E_a.gen, E_a.gsid) <_{lexicographical} \equiv E_a.gen < E_b.gen \vee (E_a.gen = E_b.gen \wedge E_a.gsid < E_b.gsid)$.
\end{displayquote}
...
Note that the definition of the replay order does not account for overflows of $gen$ or $gsid$.
This condition must be handled in software, e.g., by temporarily destroying the log of the dataset and creating a new one with a fresh log GUID.
Our implementation does not handle overflow because even with the (absurdly) conservative of \SI{1}{ns} write time per log entry, the first overflow event would only occur in \SI{584}{years} if $gen$ starts at $1$.
...
For an entry $E$, the counter $E.C_{ctxg_i} := \#{ D : D.gen < E.gen \wedge D.txg = ctxg\_i}$, i.e., the counter stores the number of entries that were written to the log since its inception, for a DMU transction with txg $ctxg\_i$, until generation $E.gen$.
...
After a txg $t_{synced}$ has been synced, PRB garbage-collects all entries $\mathcal{E}_{gc} := { E : E.txg \le t_{synced}}$.
...
In fact, since txgs never skip a number, we only store the value of $txg_{open}$ and compute $txg_{quiescing} = txg_{open} - 1$ and $txg_{sycing} = txg_{open} - 2$ on demand.
\todo{just an idea,  not in impl}

We use the same implementation to compute (re-compute) counters on the write (recovery) path.
...
Note that the use of 4-ary arrays allows the use of \textit{bitwise-and} for indexing, which is a common ZFS idiom (\lstinline{table[txg&4}).
\end{itemize}
...
The case $E < E_{last}$ (replay ordering) is prohibited because the API contract for log writers is that generation numbers must be monotonically increasing.
We consider the log corrupted if such an entry $E$ is found.
If $E.gen = E_{last}.gen$, the writer did not start a new generation and we leave the \textit{last table} unmodified.
If $E.gen > E_{last}.gen$, we create a copy of the \textit{live table}.
We refer to this snapshot as \textit{last table}.
Regardless of whether a new generation was started or not, we then update $E_{last} \leftarrow E$ and increment the counter in the \textit{live table} as described in the previous paragraph.

By definition, the counters $E.C_{txg_i}$ are the same for all entries in generation $E.gen$.
We compute them from the generation's \textit{last table} as follows:
\begin{enumerate}[noitemsep]
...
\todo{figures need some rework, they still use the 'live' and 'last' table approach to explain everything}
...
\subsubsection{Underlying Model For Data Corruption}
The use of counters to validate that no log entries are missing elies on the assumption that random data corruption is incapable of ``forging'' new log entries.
...
\subsection{Crash-Consistent Recovery}\label{di:prb:ccrecovery}
...
Replay is more complicated by two aspects:
\begin{itemize}[noitemsep]
    \item Replay of an individual log entry is not idempotent.
    \item Log replay is spread across several transaction groups and hence not atomic.
...
\begin{description}[noitemsep]
    \item[Live Table] The \textit{live table} at the time the entry was replayed, i.e., $R_i$, $i \in {0,1,2,3}$.
...
\begin{enumerate}[noitemsep]
    ...
    \end{itemize}
    \item Replay recovers \textit{replay state} $C$ from the checkpoint stored in the ZIL header.
...
\begin{description}[noitemsep]
    ...
    \item[Zero Padding] All unused bytes in the header are zero-padded. The padding is part of the header checksum.
\end{description}
...
\section{PRB Scan / Chunk Traversal}\label{di:prb:traversal}
...
\begin{description}[noitemsep]
    ...
    \item[Commit Slot Aquisition \& Chunk Replacement]
        Commit slots and the chunk lists are PRB-wide resources that are shared by all HDLs.
...
\subsection{Compute HDL-Scoped Metadata}
...
Thus, this subsection only addresses scalability concerns.

First of all, HDL-scoped metadata does not pose a scalability concerns if only one thread writes entries to the HDL.
...
For the ITXG bypass for ZVOLs (Section~\ref{sec:itxgbypass}), the ZVOL dataset's HDL is written in parallel.
...
\section{API Overview}\label{di:prb:api} % API Overview?
