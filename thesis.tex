...
\end{lstlisting}
    ...
    % The first sizeof(entry) bytes of a chunk are an entry header.
    % E := pointer to those first sizeof(entry) bytes
    % loop:
    % Otherwise:
    % If the txg stored in the header is less than min_txg, move to the next chunk (and ignore all entries in the current chunk; insertion guarantees that they have the same txg as E).
    % Get the body length from E, checksum the body. If the body checksum does not match the one we found in the header, move to the next chunk (and drop all entries in the current chunk).
    % Otherwise:
    % Invoke callback with E.


% The internal C API for iterating over a physical chunk is as follows:
% \begin{lstlisting}
...
% void prb_chunk_iter_init(uint8_t *base_pmem, size_t len,
...
% prb_chunk_iter_result_t
% prb_chunk_iter(prb_chunk_iter_t *iter, uint8_t **entry);

% // Example

% prb_chunk_iter_t iter;
% prb_chunk_iter_init(..., ..., &iter);
% uint8_t *entry;
% prb_chunk_iter_result_t res;
% while ((res = prb_chunk_iter(&iter, &entry)) == PRB_CHUNK_ITER_OK) {
%     ...
% }
% if res != ... 
% \end{lstlisting}
...
% Algorithm: pss_ring_traverse(base, len, min_txg, callback, arg)
% For each chunk (we know its start because chunks are fixed size), traverse its entries:
% The first sizeof(entry) bytes of a chunk are an entry header.
% E := pointer to those first sizeof(entry) bytes
% loop:
% If E is invalid (zero fields, header checksum error), move to the next chunk (if it’s a bitflip, we now lose all remaining entries of this chunk)
% Otherwise:
% If the txg stored in the header is less than min_txg, move to the next chunk (and ignore all entries in the current chunk; insertion guarantees that they have the same txg as E).
% Get the body length from E, checksum the body. If the body checksum does not match the one we found in the header, move to the next chunk (and drop all entries in the current chunk).
% Otherwise:
...
% in a single 
% need to iterate over the entries in a chunk in order to discover which entries have been logged before the system crash, and to decide which entries need to be replayed.

% In this section, we describe the \textit{traversal} algorithm that implements this functionality for a single chunk.
% Scanning the entire PRB for entries is then a mere application of this algorithm to all chunks added to the PRB.
