...
Note that reads and writes VDEV labels\todo{need to explain this? short analogy: partition header} are located outside of the allocatable space.
...
\section{ZIL kinds}
Coexistence with ZIL-LWB and preservation of ZFS's crash consistency guarantees are two hard requirements for ZIL-PMEM (see Section~\ref{sec:requirements}).
In this section we describe how we re-architected the existing ZIL code to address both of these problems.

In upstream ZFS, the \textit{zil.c} code module implements all ZIL functionality.
