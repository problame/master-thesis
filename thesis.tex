...
Recovery is split into the claiming phase where we discover replayable log entries for all datasets, and the replay phase were we replay an individual dataset's unsynced log entries.
As explained in the API overview\todo{chapter name} in Section~\ref{di:prb:api}, these operations are scoped to a single HDL.
Thus, unless noted otherwise, we describe the recovery procedures for a single HDL.
...
In the scanning phase, the algorithm uses the traversal algorithm (Section~\ref{di:prb:traversal}) on a provided set of chunks to search for log entries that have the HDL's \textit{log GUID}.
It wraps these candidate entries in a \textit{replay node} and collects the nodes in a b-tree.
After the scan is complete, the b-tree contains a superset of nodes that might need to be replayed.
In the \textit{iteration phase}, the algorithm iterates over the b-tree's nodes in replay order.
For each entry, the algorithm checks whether the entry needs to be replayed and if so, invokes a user-defined callback.
...
For example, the replay nodes $(2,1), (1,1), (1,3)$ are ordered as $(1,1), (1,3), (2,1)$.
The write path guarantees that there are no entries with the same $(gen, gsid)$.
(Remember that generations are defined as unique in a HDL log and generation-scoped IDs as unique within a generation).
However, a number of circumstances can cause such a collision to occur:
\begin{enumerate}[noitemsep]
    ...
    \item Reuse of \textit{log GUIDs}.
      This is very unlikely because we generate them as 128 bit random numbers.
       However, future work could systematically prevent log GUID collision in PMEM by traversing all chunks to find used log GUIDs and check for collisions when issuing new log GUIDs.
...
In the iteration phase, the algorithm keeps all of its state in the \textit{replay progress} structure.
It contains the following variables:
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    \item[precrash-txg] The last transaction group that was completely synced before the system crashed.
    \item[$\mathbf{(gen_{last}, gsid_{last})}$] The $(gen, gsid)$ of the last replay node that was last replayed.\todo{use 'visited' or 'iterated upon' here? if so, fix usage in rest of paragraph}
        A value of $(0,0)$ means that no replay node has been replayed.
    \item[validating counter tables] The same two tables that we use on the write path to compute the entries' \textit{Counters Table}.
\end{description}
The purpose of the replay progress structure is to keep track of the log entries that have already been replayed.
We skip an entry if its $txg \le precrash\text{-}txg$ or its $(gen,gsid) \le (gen_{last}, gsid_{last})$.
Otherwise, the entry is eligible for replay iff all the changes that it logically depends on have been applied to the dataset.
This is equivalent to checking that all entries with $txg > precrash\text{-}txg$ have been replayed.
We use the \textit{validating counter tables} to check this property.
For every entry that is eligible for replay, we first update the validating counter tables just as we do on the write path:
if the entry starts a new generation, we compute the \textit{last table} from the \textit{live table}.
And regardless of whether a new generation was started or not, we then increment the \textit{live table} counter.
In a second step, we ensure that the \textit{last table}'s counters match the counters stored in the entry.
We must only check the counters for $txg > precrash\text{-}txg$ since we skipped all entries in $txg \le precrash\text{-}txg$ (see above).
If the checked counters match, we know that we have not lost entries up to but not including the current entry's generation.
Thus, all logical dependencies of the entry are satisifed and we can invoke the callback.
Conversely, if we lost an entry, we do not invoke the callback and return an error which contains the current entry and the replay progress state as a \textit{witness}.
There are several corrolaries that result from\todo{language} this algorithm:
\begin{enumerate}[noitemsep]
    ...
    \item Thus, all other entries $(gen_l,  \_)$ in a generation with a lost entry are replayed before the error is detected.
        This is correct and desirable because by definition, entries within a generation have no logical dependency on each other (see Section~\ref{di:prb:structure}).
...
Both the scanning and iteration phase are resilient against online data corruption in PMEM.
The reason is that they only operate on DRAM-buffered entry metadata in the replay node.
Further, if the callback wants to read the entry, it is forced to do DRAM-buffering as well:
the callback does not receive a pointer into PMEM but a pointer to the (opaque) replay node struct.
The callback must use the \lstinline{zilpmem_prb_replay_read_replay_node} function to copy the entry into a DRAM buffer which
\begin{itemize}[noitemsep]
...
\end{itemize}
The callback must propagate errors returned from the function.
If the callback returns an error, the iteration is stopped.
