...
\begin{enumerate}[noitemsep]
    ...
    \item Virtualize calls of \lstinline{zillwb_} functions:
          \begin{itemize}
              ...
              \item Replace all calls to \lstinline{zillwb_FN} functions in \textit{zil.c} with indirect calls through the vtable, i.e., \lstinline{zilog->zl_vtable.FN}.
          \end{itemize}
    \item Make non-ITX state private to \textit{zil\_lwb.c} by turning it into a subobject.
          ...
          \end{itemize}
\end{enumerate}
The end result ist best described in the terminology of object-oriented languages.
\textbf{\lstinline{zilog_t} is an abstract baseclass} that implements the public ZIL interface and ITX-related functionality and defines abstract methods for persisting log records which must be implemented by concrete subclasses.
\lstinline{zilog_lwb_t} is such concrete subclass that implements the LWB-based persistence strategy.
...
The subclass that is used at runtime is determined by the \lstinline{zh_kind} field in the ZIL header (see Section~\ref{sec:di:zil_header} and Figure~\ref{lst:zil_header_before_and_after}).
...
The idea behind this approach is to keep administration simple, which is a defining paradigm for ZFS that want to uphold for ZIL-PMEM (ref. Section~\ref{sec:requirements}).
...
\begin{enumerate}[noitemsep]
    ...
    \item \lstinline{zil_close()} and subsequently free the currently open \lstinline{zilog_t} instances.
...
Instead, we add a kernel module parameter \lstinline{zil_default_kind} that defines the ZIL kind that is used for the \lstinline{zh_kind} field in new dataset's ZIL headers.
There is no mechanism to change \lstinline{zh_kind} over the lifetime of a dataset.
Also, this stop-gap solution makes it possible to mix different ZIL kinds in the same pool which works correctly but is undesirable from the perspective of simple administration where ZIL kinds are transparent to the user.
...
Given our insights into ZIL traversal, we implement the following change as a precursor to the refactoring of \lstinline{zilog_t}.
We change the \lstinline{zil_parse} API to work on a \lstinline{const zil_header_lwb_t*} and rename it to \lstinline{zillwb_parse_phys} to reflect the fact that it is specific to ZIL-LWB.
We also split up the \textit{dmu\_traverse} API into separate functions for generic pool and ZIL-LWB chain traversal so that it is always clear when a piece of code has ZIL-LWB specific behavior.

\subsection{ZIL-LWB-Specific Callbacks}
...
\begin{description}
    \item[zil\_lwb\_add\_txg] Necessary to keep the LWB in-DRAM representation alive when writing \lstinline{WR_INDIRECT} blocks.
    ...
    \item[zil\_bp\_tree\_add] Durign a ZIL traversal with \lstinline{zil_parse} (or \lstinline{zillwb_parse_phys}, see previous\todo{ensure} section), this API is used to avoid doing operation more than once for a given blockpointer.
