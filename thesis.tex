...
% The basic problem with the lifecycles is that the chunk allocation state
% for a given PMEM vdev is excluisvely kept in DRAM. It has the same
% lifetime as a vdev. Then there's also state per objset (which chunks
% the objset has claimed) that needs to be kept in DRAM until we export
% the pool or do replay or destroy the (physical) objset.

% The per-PMEM-vdev state kept in-DRAM has lifetime MIN(spa_t in-DRAM, vdev healthy).
% That's struct zilpmem_prb_t and the wrapping spa_prb_t.

% The per-objset state (chunks held during claiming) needs to live
% MIN(spa_t in-DRAM, objset existence). It cannot be stored in the
% objset_t because objset_t are frequently allocated and freed in
% dmu_objset_{hold,rele}, particularly during pool import.

% We solve this by storing the per-objset state in the spa_prb_handle_t's
% which live in spa_zilpmem. spa_prb_handle_t's wrap
% zilpmem_prb_handle_t's.
% spa_prb_handle_t's are alloced during claiming or when a dataset is
% created.
% spa_prb_handle_t's are freed when a dataset is destroyed or when the
% spa is unloaded. During spa unload we ensure that the claimed chunks
% are not overwritten.

\section{ZILOG\_PMEM\_T}\label{sec:zilpmem:zilog}
The role of \lstinline{zilog_pmem_t} is to adapt between the ZIL domain and the domain.
