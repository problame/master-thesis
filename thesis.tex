...
When the system call handler finishes the DMU transaction by calling \lstinline{dmu_tx_commit(T_i)}, the logical change $C_i$ that $T_i$ makes is not yet persisted to stable storage.
...
At this point, the ZIL-kind specific implementation takes over: it is responsible for appending the commit list to the previously written lists in some form of stable storage.
...
The invocation order is given by the commit list, but entries for transactions before or in \textit{precrash-txg} must be skipped because the change encoded in them is already part of the dataset state.
...
\begin{itemize}[noitemsep]
\item On the write path, it needs to append entries to a sequential log that is synchronously persisted to stable storage.
...
It is set up with many segments of PMEM (\textit{chunks}) that it uses for storage.
For each dataset, it exposes a \textit{HDL} which implements a (virtual) log persisted in PMEM.
The log is created when dataset is first mounted.
Then, it is available for writing \textit{log entries} to it.\todo{language... help}
...
The log is never read except during recovery after the system crash.
...
The motivation for this feature is that, although a sequential chain is sufficient for the ZIL's \textit{commit list} model, the sequentiality inherent in the list-based model is a scalability bottleneck.
...
Due to the support for parallel writes, the logical structure of the log is more complicated than the concatenated commit list introduced in the previous section.
...
The most recent generation in the log is called the open generation.
...
If no new generation is started, the log entry is allowed to be written in parallel with other threads that write to the open generation.
...
We start the PRB lifecycle starts by allocating it using the \lstinline{zilpmem_prb_alloc} function.
...
The next step is to give PRB access to segments of PMEM in the form of \textit{chunks}.
A chunk is a contiguous segment of kernel virtual address space that maps directly to PMEM.
We allocate the chunk using \lstinline{prb_chunk_alloc} and add it to the PRB using the \lstinline{zilpmem_prb_add_chunk_for_write} or \lstinline{zilpmem_prb_add_chunk_for_claim} functions.
When the PRB is constructed for the first time, we use \lstinline{_for_write}.
Otherwise, we use \lstinline{_for_claim} to indicate that the chunks need to be held back for claiming and replay.
...
The PRB consumer is responsible for ensuring that the PRB is initialized with the same chunks of PMEM for its entire lifecycle.
...
In ZIL-PMEM, we currently simplify this task by hard-coding a fixed chunk size (\SI{128}{MiB}\todo{check}) and partitioning the PMEM SLOG vdev's allocatable space into contiguous segments of that size.
...
\textbf{Note} The \lstinline{zilpmem_prb_claim} function must be called every time the pool is imported because it is responsible for determining which of the chunks that were added using \lstinline{zilpmem_prb_add_chunk_for_claim} actually need to be held back for replay.
...
It validates the PRB-managed checksum of the log entry's body.
Also, it ensures that machine-check exceptions (MCEs) raised\todo{caused?} by PMEM are converted to errors.

\lstinline{zilpmem_prb_replay} can fail either due to an error in the callback or because log entries are missing.
The result type returned by the API contains the log entry where the missing log entry that prompted the failure.
The caller decdes whether to retry replay or abandon the unreplayable part of the log\todo{retry with an override flag that discards the unreplayable part of the log}.
...
The caller provides the entry body as opaque byte slice is stored verbatim in PMEM.
...
In this section we describe PRB's persistent data structure.
It encodes the content of each HDL's logs, their logical structure and the HDL state that is necessary for claiming and crash-consistent replay.
...
From PRB's perspective, chunks are mere slices of PMEM defined by a kernel-virtual \textbf{base address} and their \textbf{length}.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    ...
    \item[Generation-Scoped ID (gsid)] Within a generation, we identify and order entries by a unique unsigned 64-bit non-zero integer called \textit{generation-scoped-id} (GSID).
...
\end{description}
The Counters Table is the same for all entries in a generation because it does not count the entries of the currently open generation.
However, we currently replicate it in each entry to minimize coordination between parallel writers.
...
However, not every aspect of the runtime state of HDL is reflected in it, but only the information needed for claiming and replay.
