...
\textit{zil-pmem} follows \textit{async} latency's almost flat curve at all percentiles until the expectable knee at \lstinline{numjobs=4}.
In contrast, \textit{zil-lwb}'s 99.9th and 99.99th percentile show steadier but steeper growth with rising \lstinline{numjobs}.
...
We execute each benchmarks on top of different \textit{storage stacks} that represent alternatives to ZFS with ZIL-PMEM and compare the results in the subsequent sections.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    \item[zfs-\{lwb,pmem,sync\_disabled\}] A single ZFS dataset created on a zpool with the familiar hardware configuration (30 striped NVMe partitions and \texttt{/dev/pmem} as SLOG).
        ...
        All variants use a \lstinline{recordsize=4k} and \lstinline{compression=off} on the dataset.
        The \textit{zil-pmem} variant configures PRB with \lstinline{ncommitters=3}.
    \item[\{xfs,ext4\}\_\_on\_\_\$BDEV\_\_dax\_\$DAX] Linux 5.9's \textit{xfs} or \textit{ext4} deployed on the \textit{block device stack} \lstinline{$BDEV}.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    \item[devpmem] The \lstinline{fsdax} namespace block device in devfs, i.e., \texttt{/dev/pmem0}.
    \item[dm-writecache]
    ...
    \item[zvol-lwb-rs\_\{0,1\}]
        ...
        The \textit{rs} variable control the value of the \lstinline{zvol_request_sync} tunable which, if enabled, processes block I/O requests (\lstinline{struct bio}) synchronously instead of submitting them to a thread pool.
    ...
    \item[zvol-pmem-rs\_\{0,1\}-byp\_\{0,1\}-nc\_3] Like \textit{zvol-lwb}, but configured with the ZIL-PMEM ZIL kind.
        PRB's \lstinline{ncommitters} tunable is always set to $3$.\todo{need explain why?}
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    ...
    \item[zfs-pmem] ZFS with the ZIL-PMEM ZIL kind and \lstinline{ncommitters=3}.
        Note that \lstinline{zvol_request_sync} and the ITXG bypass are only relevant for ZVOLs and thus are not part of the configuration matrix.
\end{description}

We compare the performance of the storage stacks with the benchmarks described below.
To facilitate\todo{unify?} the comparison, all benchmarks have the following properties:
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    \item[Scaling factor] Each benchmark has a scaling factor that increases the degree of concurrent synchronous I/O operations issued by the application.
    ...
    \item[Result Metric] Each benchmark reports a single result metric per run.
        This allows us to compare the performance of a benchmark across different storage stacks and scaling factor values.
        For all benchmarks in this evaluation, a numerically greater result metric is better.
\end{description}
What follows is a description of the benchmark name used in text and plots as well as a description of the workload.
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    \item[fio-growing] The 4k synchronous random write workload that motivated our work (see previous section and Section~\ref{ch:lwb_analysis:setup}), but with all fio job files within the same filesystem.
        The scaling factor is the fio \lstinline{numjobs} parameter which control the number of threads that perform I/O.
    ...
    \item[fio-fixed] The 4k synchronous randomm write workload, with all files on a single filesystem, but a fixed working set size, independent of \lstinline{numjobs}.
        We achieve the fixed working setting the \lstinline{size} parameter to $\frac{1~\text{GiB}}{numjobs}$.
        We report mean IOPS as the result metric.
    ...
    \item[filebench oltp] The filebench \textit{oltp} workload emulates database workloads.
        ...
        It is our understanding that scaling the number of log writer threads to a number greater than one would not be feasible in practice and thus be unrealistic.
        We use a runtime of 20 seconds and report filebench's \textit{ops per second} value as the result metric.

    \item[MariaDB/sysbench] We deploy the MariaDB 10.5.9 Docker image in its default configuration (InnoDB storage engine) with \texttt{/var/lib/mysql} directory bind-mounted to the storage stack's mountpoint.
        ...
        We use Docker's \lstinline{--net=host} parameter when starting the MariaDB container to allow sysbench to connect via loopback TCP to avoid the overhead the overhead of NAT and Docker's user-space proxy.
        The result metric is the number of transactions per second reported by sysbench.
    ...
    \item[Redis-SET] Redis is a popular in-memory key value store.
        ...
        First, the system periodically persists a snapshot of the Redis databased (RDB).
        ...
        The scaling factor controls the number of parallel clients via the \lstinline{--threads} and \lstinline{-c} parameters.
        We use the \textit{SET} workload to perform $10^6$ \textit{SET} operations with a random key drawn from a keyspace of the size $10^6$ to reduce contention at the data level.
    ...
    \item[RocksDB-fillsync] RocksDB is a popular key-value store developed by Facebook that is optimized for fast storage devices.
        ...
        We measure the impact of ZIL-PMEM on RocksDB WAL performance with the \textit{fillsync} benchmark in RocksDB's \textit{db\_bench} tool.
...
We first compare the speedup of ZIL-PMEM vs ZIL-LWB with ZIL-LWB as the baseline.
...
The highest speedups are achieved at scaling factor $1$ by the fio workloads ($7.1$, $5.3$) and \textit{RocksDB-fillsync} (100k ops-per-sec, speedup of $5.8$).
\textit{Redis-SET} achieves shows a speedup of $2.68$ (30k rps) and MariaDB achieves 10k tps which is a speedup of $2.14$.
For scaling factor $4$, the fio workload and RocksDB speedup declines whereas Redis grows to $2.77$ and MariaDB grows to $3.08$.
For scaling factor $8$, all workloads show a reduction in speedup (RocksDB $2.8$, Redis $1.9$, MariaDB $1.4$).
...
Since PMEM I/O appears as CPU-bound activity towards the thread scheduler, the single log writer thread might have competed with the reader processes.
...
A possible explanation for the meager speedup is the larger data volume (16k appends) compared to the fio workloads (4k writes).
In combination with the large amount of metadata (file creation and deletion), ZIL-LWB might also be benefitting from the \textit{commit ITXs'} pipelininig behavior.
...
Future work should investigate ZIL-PMEM's behavior during this benchmark in detail and determine how beneficial pipelining akin to \textit{commit ITXs} would be.
...
Both XFS and Ext4 are DAX-aware and thus we inlcude both with and without the \textit{dax} mount option.
...
Note that that the ZFS configurations only use PMEM for the ZIL and use the NVMe devices for permanent storage, the Linux filesystem configurations are PMEM-only.
...
Note that without the \textit{dax} mount option, a significant amount of parallelism (scaling factor $8$) is required to achieve similar results in the fio workloads.
...
\footnote{The RocksDB documentation and comments in the source code suggest that the WAL files are written in 32k blocks that are zero-padded if necessary~\cite[db/log\_writer.h]{RocksDBGitHubWikiWalPerformance}.
    We observed the contrary behavior using \lstinline{strace}: each \textit{Put} operation with \lstinline{WriteOptions.sync} results in a \lstinline{write} and \lstinline{fdatasync} system call. The \lstinline{write} system call's data size is proportional to the sum of the \textit{Put} operation's key and value sizes.}
ZIL-PMEM handles this worklaod efficiently than the Linux filesystems because the ZIL's data journaling causes less write amplification than the Linux filesystems whose latency is likely dominated by page-granular write-back cause by \lstinline{fsync}.

For the \textit{MariaDB} benchmark, the DAX-aware Linux filesystem configurations achieve up to 1.6x more transactions per second than ZIL-PMEM.
The \lstinline{sync=disabled} workload is on-par with the non-DAX-aware configurations, at speedups of ca. 1.1, 1.2 and 1.3 at scaling factors 1, 4, and 8.
These results suggest the existence of a bottleneck outside of the filesystem implementation.
A qualitative inspection of the CPU utilization shows a signficant amount of context switching but no clearly identifiable CPU bottleneck.
...
Regarding the \textit{filebench-oltp} benchmark, no configuration performs exceptionally better than another, including \lstinline{sync=disabled}.
Again, we suspect the high CPU utilization of the 200 reader processes to be the true bottleneck in this benchmark.

For \textit{filebench-varmail}, the highest speedup at scaling factor $1$ is 1.35 achieved by XFS in DAX-aware mode, followed by ZFS with \lstinline{sync=disabled} with 1.22.
For higher scaling factors, the speedups are slightly lower.
Knwoing that no configuration is able to achieve a speedup as dramatic as observed for the \textit{fio} workloads is an additional indicator that \textit{filebench-varmail} performance is likely not bounded by ZIL-PMEM's implementation.
...
In contrast, the Linux filesystem configuration store all their data on PMEM.
A fairer comparison is thus to deploy the Linux filesystems on the \textit{dm-writecache} stack with a \textit{dm-stripe} of the NVMe partitions as the origin block device.
...
\begin{figure}[H]
    ...
    \caption{
        Comparison of Linux filesystems on dm-writecache to ZIL-PMEM as a baseline.
    }
\end{figure}

ZFS with ZIL-PMEM performance is within +/-30\% of XFS on \textit{dm-writecache} at all scaling factors in all benchmarks except \textit{Redis-SET} and \textit{rocksdb-fillsync} where ZIL-PMEM performs significantly better.
The 30\% spread decreases slightly with increasing scaling factor but does not change the overall impression.
Ext4+dm-writecache often performs signficantly worse than ZIL-PMEM (and XFS+dm-writecache!), with only 45\%, 28\% and 14\% of ZIL-PMEM's performance in the \textit{filebench-varmail}, \textit{Redis-SET}, and \textit{rocksdb-fillsync} benchmarks at scaling factor $1$.
Our explanation for ZIL-PMEM's better performance in the \textit{Redis-SET} and \textit{rocksdb-fillsync} workloads is the low write amplification of the ZIL's logical write-ahead logging compared to the block-sized granularity of dm-writecache.
Note that this is a fundamental advantage of ZFS's integrated design (filesystem and volume manager) that cannot be attained by the layered design of the Device Mapper stack.
Ext4's under-performance in the \textit{filebench-varmail} benchmark could also be observed in the previous sections and must be due to another factor.

We have manually observed the system during while benchmarking the \textit{dm-writecache} stack to ensure that the comparison with ZFS is actually fair.
...
The two \textit{fio} workloads were the exception but the write load toward the NVMe devices was well below their maximum capacity.
...
ZIL-PMEM has similar problems (125k at factor 1, 217k at factor 4) albeit due to the serial structure of ZIL chain.
...
The results are relative to \textit{zvol-pmem-rs\_0-byp\_0-\_nc3} as a baseline which is the standard configuration for ZIL-PMEM pools.
Note that deploying a filesystem on a ZVOL generally makes little sense since ZFS filesystems provide the same basic service with more optional features at less overhead.
However, ZVOLs are a popular choice for compute \& storage virtualization where the ZVOL is used as a virtual machine's virtual hard disk or as an iSCSI target.
To simplify our evaluation, we avoid the overhead of a hypervisor/iSCSI and instead instantiate XFS directly on top of the ZVOL, mount it, and execute the benchmarks.
...
At scaling factor 1, the speedup over ZIL-LWB is approximately 2x for all workloads except \textit{MariaDB} (1.5x) and \textit{filebench-oltp} (1.05x).
At higher scaling factors, the speedup declines for all benchmarks albeit less so for \textit{Redis-SET} and \textit{RocksDB-fillsync} than the other workloads.

The effect of \lstinline{zvol_request_sync=1} is ambiguous and not very signficant for ZIL-LWB but very beneficial for ZIL-PMEM in the \textit{fio} workloads at scaling factor 1 with a 2x and 2.6x speedups over standard configuration, respectively.
\textit{MariaDB}, \textit{Redis-Set} and \textit{RocksDB-fillsync} also benefit with speedups of 1.18x, 1.23, and 1.30 over the standard configuration.
...
We also conducted the benchmarks with Ext4 instead of XFS which showed worse performance in almost all configurations if \lstinline{zvol_request_sync} was set, with the exception of \textit{fio}.
...
For example, for \textit{fio-growing}, at scaling factor 8, IOPS increase from 84k to 102k (21\%) for \lstinline{zvol_request_sync=0} and from 117k to 159k (35\%) with \lstinline{zvol_request_sync=1}.
For Ext4 (not shown in the figure), \textit{filebench-varmail} also shows a 20\% improvement with enabled ITXG bypass and \lstinline{zvol_request_sync=0}.
In contrast to \lstinline{zvol_request_sync=1}, activation of the ITXG bypass does not appear to have a negative impact for XFS.
For Ext4, enabling ITXG bypass causes some performance degradations if \lstinline{zvol_request_sync=1}.
...
This is particularly noticable for the \textit{fio} workloads, \textit{Redis-SET} and \textit{RocksDB-fillsync}.
For example, the speedups achieved by the \textit{fio} workloads and \textit{RocksDB-fillsync} on ZFS vs. XFS+ZVOL, both with ZIL-PMEM, halved for scaling factor 1.
Write amplification cannot be the explanation because \textit{fio}'s 4k writes are unlikely to be amplified whereas \textit{RocksDB-fillsync}'s writes are guaranteed to be amplified to the emulated block size (4k).
The more likely cause is mere overhead added by the additional software layer.
For example, the \textit{fio-growing} workload, which is purely latency-bound, achieves 126k IOPS at scaling factor 1 on ZFS+ZIL-PMEM but only 28.6k IOPS on XFS+ZVOL+ZIL-PMEM.

% TODO:
% more detailed itxg bypass evaluation?
...
The goal is to avoid wasteful on-CPU stall cycles which inevitable occur if the aggregate write bandwidth to PMEM exceeds the hardware capacity.
(See Section~\ref{sec:requirements} and \ref{di:prb:write} for a more detailed explanation.)

To determine the mechanism's effectiveness, we use our 4k synchronous random write workload with private ZFS filesystems per fio thread.
We add low-overhead per-CPU counters to ZIL-PMEM to sum up the time $T_{pmem}$ that is spent writing PMEM as well as the number of write operations $N_{ops}$.
We then compute the \textit{average PMEM write time per IOP} $T_{pmem_{iop}} = \frac{T_{pmem}}{N_{ops}} [\frac{ns}{op}]$.
The most efficient value for \lstinline{ncommitters} maximizes average IOPS (reported by fio) while minimizing excessive of $T_{pmem_{iops}}$.
...
On both PMEM configurations, we create a \SI{40}{GiB}-sized \textit{fsdax} namespace which we use as the PMEM SLOG for ZIL-PMEM.
We use a machine with higher core count.
...
\begin{description}[noitemsep]
    ...
    \item[fio] fio-3.21
\end{description}
As with our main evaluation system (Section~\ref{ch:lwb_analysis:setup}), we leave SMT enabled.
Since NUMA-concerns are out of scope for this thesis, we disable the second socket in software using the \lstinline{isolcpus=18-35,54-71} kernel command line parameter.
Because the system does not have NVMe devices available, we provision the disabled socket's Optane DIMMs in \lstinline{AppDirectNotInterleaved} move with one PMEM region and a single \lstinline{fsdax} namespace per DIMM.
...
\begin{figure}
    ...
    \caption{Comparison of performance and PMEM write time per IOPS for different values of $ncommitters$ in two PMEM configurations (left non-interleaved, right four-way interleaved).}
    \label{fig:eval:ncommitters_scalability:main}
\end{figure}

Figure~\ref{fig:eval:ncommitters_scalability:main} visualizes the results of this experiment.\todo{need a better figure}
For the non-interleaved DIMM, we observe that the system's peak IOPS is 400k, first achieved with $ncommitters \ge 3$.
For $ncommitters = 3$, the peak is at $numjobs = 8$, followed by a decline to 263k IOPS for higher $numjobs$.
With $ncommitters = 8$, the system is able to sustain the 400k IOPs for the for $numjobs \in 7 \dots 13$.
The price for this sustained throughput at $ncommitters = 8$ is paid in significantly more on-CPU PMEM time per IOP:
looking at $numjobs = 8$, the $T_{pmem_{iops}}$ is $3300, 4300, 6500~\text{ns}$ for $ncommitters = 3, 4, 8$.
Moving to higher $numjobs = 12$, $T_{pmem_{iops}}$ climbs to $11600, 20600~\text{ns}$ for $ncommitters = 8, 12$ whereas $ncommitters = 3$ is successfully limited to $2700~\text{ns}$ of PMEM write time per IOP.
The $ncommitters = 12, 24$ configurations do not achieve higher peak IOPS than $ncommitters = 8$ but decline to lower values for high $numjobs$, e.g., only 237k IOPS for $ncommitters = 24, numjobs = 18$.
...
We achieve the highest IOPS for the configuration where the \textit{commit slot} mechanism has no effect, i.e., $ncommitters = 24, numjobs = 18$ with 900k IOPS.
Given that the IOPS curve has not reached a plateau at this point, it may be possible that even more IOPS are possible at higher $numjobs$.
$T_{pmem_{iops}}$ is the same for all $ncommitters$ values at a given $numjobs$ value (+/- $100~\text{ns}$).
It is $1660~\text{ns}$ for $numjobs = 1$ and grows slightly unevenly towards $2560~\text{ns}$ at $numjobs = 24$.
This is an increase of merely 54\% which is likely to be acceptable in any setup, given the 9.6x increase in IOPS (93k to 900k) that is possible with $ncommitters = 24$.

To determine the degree to which the \textit{commit slot} abstraction impacts performance in $ncommitters > numjobs$ configurations, we add additional instrumentation that measures the average latency of commit slot aquisition and release.
Figure~\ref{fig:eval:ncommitters_scalability:overhead} visualizes the results for $ncommitters = 24$.
The absolute overhead is approximately the same for both PMEM comfigurations.
It starts at approximately $100~\text{ns}$ for $numjobs = 1$, climbs to $240~\text{ns}$ at $numjobs = 4$, and then scales linearly to $385~\text{ns}$ at $numjobs = 18$.
...
Thus, without \textit{commit slots}, \lstinline{zil_commit} could be $\frac{1}{1 - 0.13} = 14.9\%$ faster.
However, due to the overhead of the other ZFS components, the contribution to overall IOP latency is only 2\%, which is a rather meager improvement.
\todo{Think this through again, I think it's correct, but should be double-checked.}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{fig/evaluation/ncommitters_scalability__overhead}
    \caption{
        Overhead of the \textit{commit slot} for $24 = ncommitters > numjobs = 18$.
...
\begin{itemize}[noitemsep]
    ...
    \item The \textit{commit slots} mechanism's multi-core scalability is acceptable and contributes little relative overhead to each IOP compared to non-ZIL ZFS components.
    \item Single Optane DIMM in non-interleaved mode:
        \begin{itemize}
            \item Our 4k synchronous write workload is able to achieve peak performance with 400k IOPS as early as $ncommitters = 3$.
            \item Setting $ncommitters = 8$ extends the peak to a plateau with regards to $numjobs$, but consumes $\frac{6500~\text{ns}}{3300~\text{ns}} = 1.96$ times more PMEM write time per IOP.
            \item Note that we use $ncommitters = 3$ as the default configuration for ZIL-PMEM in all of the previous benchmarks.
                Our rationale is that an in-kernel filesystem should not waste CPU time on PMEM I/O because it is primarily one of many OS services and not the main purpose of the system.
                If the system's primary role is that of an NFS/SMB share, or that of an iSCSI target backed by ZVOLs, it may be appropriate to adjust this policy.
        \end{itemize}
    \item Four interleaved Optane DIMMs:
        \begin{itemize}
