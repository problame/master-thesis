...
ZFS's on-disk structure is a tree of SPA-allocated blocks that are linked to their children through \textit{block pointers}.
...
Regarding the uberblock, the specific \lstinline{objset_phys_t} structure that it points to is that of the MOS whose object's contains the DSL dataset metadata.
...
It is not used for any new DMU transactions (that's the open txg's role), but some of the DMU transactions assigned to it have not yet completed.
However, once that has happened, it is guaranteeed that the accumulated dirty state for the quiescing txg is not going not change and ready to be written out.
...
It is of crucial importance that the completion of a DMU transaction, indicated by a call to \lstinline{dmu_tx_commit}, does \underline{not} guarantee persistence.
...
To guarantee synchronous semantics, the DMU consumer must invoke the ZIL's \lstinline{zil_commit} API after completing the transaction but before reporting the logical change as committed to any upper layer.
...
All datasets (i.e, filesystems, ZVOLs) are effectively reset to their $precrash\_txg$ state.
...
The following figure provides an example for the algorithm above and the replay sequence for different $precrash\_txg$s.
...
Since the replay callbacks for the individual log records are not idempotent, the ZIL must ensure that, after a crash during replay, the changes that were already replayed in transaction group 42 are not replayed again.
