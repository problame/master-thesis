...
We present PRB in a top-down manner, starting with a black-box description of the logical structure for a single log in Section~\ref{di:prb:structure} followed by a walkthrough of the APIs that are used by ZIL-PMEM (Section~\ref{di:prb:api}).
Equipped with a functional understanding of PRB, we proceed with the details of the design and implementation.
In Section~\ref{di:prb:phys} we describe the persistent data structures and their invariants.
...
The PRB exposes an API that implements the conceptual model presented in the previous section.
...
\end{description}
Entries are stored in PMEM chunks as a contiguous sequence that is terminated by an invalid entry header.
However, neither the position of the entry within the sequence nor its location in PMEM space is semantically relevant.
Instead, each entry stores metadata in its header that is interpreted by PRB during claiming and replay.
\begin{description}
    \item[Attribution] We use the \textit{Log GUID}, a 128bit random number, to uniquely identify a log and to find its entries in PMEM.
    The Log GUID is repeated in the HDL's \lstinline{zil_header_pmem_t} and all entry headers written through the HDL.
    \item[Ordering \& Dependencies]  The entry headers of the entries of a log contain sufficient metadata to define a total order on them.
        The metadata is also sufficient to determine whether entries are missing.
\end{description}
