...
We extend our benchmarking suite with several application-level benchmarks to determine ZIL-PMEM's impact on real-world application performance.
We execute these benchmarks on top of different \textit{storage stacks} that we believe could be considered as alternatives to ZFS with ZIL-PMEM.
We present and discuss the results in the subsequent subsections.
...
\begin{description}[noitemsep]
    \item[zfs-{lwb,pmem,sync\_disabled}] A single ZFS dataset created on a fresh zpool with the familiar hardware configuration (30 striped NVMe partitions + 1 /dev/pmem as SLOG).
        For \textit{zfs-lwb} and \textit{zfs-pmem}, we configure the ZIL-LWB/ZIL-PMEM ZIL kind.
        For \textit{zfs-sync\_disabled}, we configure the ZIL-LWB ZIL kind but set the \lstinline{sync=disabled} property.
        All variants set \lstinline{recordsize=4k} and \lstinline{compression=off} on the dataset.
    \item[\{xfs,ext4\}\_\_on\_\_BDEV\_\_dax\_DAX] Linux 5.9's \textit{xfs} and \textit{ext4} filesystems deployed on the \textit{block device stack} \lstinline{BDEV}.
        The \lstinline{DAX} value (\lstinline{True} or \lstinline{False}) indicates whether the \lstinline{dax} mount option was set.
        We always perform benchmark runs without \lstinline{dax}, and perform an additional run if \lstinline{BDEV} is a DAX-capable device, i.e., if it is raw PMEM.\todo{make sure DAX is explained}
\end{description}
The xfs and ext4 stacks are parametrized by the \textit{block device stacks} which we define as a hardware and software configuration that ultimately provides a, potentially virtual, block device.
...
\begin{description}[noitemsep]
    ...
    \item[dm-writecache] A dm-writecache target.\todo{config}
    \item[zvol-lwb-rs\_\{0,1\}] A ZVOL exposed by a zpool in the same config as \textit{zfs-lwb}.
        We set \lstinline{volblocksize=4k} on the ZVOL dataset.
        The \textit{rs} variable controls the settings for \lstinline{zvol_request_sync}.\todo{make sure explained}
    \item[zvol-sync\_disabled] Like \textit{zvol-lwb}, but with \lstinline{sync=disabled}.
    \item[zvol-pmem-rs\_\{0,1\}-byp\_\{0,1\}-nc\_3] Like \textit{zvol-lwb}, but with ZIL-PMEM as ZIL kind.
        We set PRB's \lstinline{ncommitters=3} and expose enablement of the ITXG bypass as the \textit{byp} variable.
\end{description}
Two examples for storages stacks are:
\begin{description}[noitemsep]
    \item[ext4\_\_on\_\_zvol-pmem-rs\_1-byp\_0-nc\_3\_\_dax\_False] Ext4 deployed on a ZVOL on a zpool with the ZIL-PMEM ZIL kind.
    ...
    \item[xfs\_\_on\_\_devpmem\_\_dax\_True] XFS deployed on the \texttt{/dev/pmem0} block device, mounted with the \lstinline{dax} option.
    \item[zfs-pmem-rs\_0-byp\_0-nc\_3] ZFS with the ZIL-PMEM ZIL kind. The \lstinline{zvol_request_sync=0} setting and disabled ITXG bypass are irrelevant for ZFS.\todo{mask them out in post?}
\end{description}

All benchmarks have two common properties:
\begin{description}
    ...
    \item[Result Metric] Each benchmark reports a single result metric per run which allows to compare the performance of a benchmark across storage stacks and scaling factor values.
        All of the benchmarks presented below use a result metric where greater values represent better performance.
\end{description}
We use the following benchmarks:\todo{language}
\begin{description}
    \item[fio-growing] The 4k synchronous random write workload presented in the previous section, but with all files located on a single ZFS dataset.
    ...
    \item[fio-fixed] The 4k synchronous randomm write workload, with all files on a single dataset, but a fixed working set size, independent of \lstinline{numjobs}.
        We achieve the fixed working setting the \lstinline{size} parameter to $\frac{1~\text{GiB}}{numjobs}$.
        We report mean IOPS as the result metric.

    \item[filebench varmail] Filebench is a benchmarking engine that executes workloads described in a domain-specific language.
    ...
    \item[MariaDB/sysbench] We deploy the MariaDB 10.5.9 Docker image in its default configuration (InnoDB storage engine) and the \texttt{/var/lib/mysql} directory set tot he storage stack's mountpoint.
        ...
        We use the default of a single table and map the scaling factor to the number of threads that perform the insert queries.
        Each thread uses a private, long-lived connection to the MariaDB server.
        We use Docker's \lstinline{--net=host} parameter to start the MariaDB container to allow sysbench to connect via loopback TCP without and use the\lstinline{--net=host} parameter to avoid proxying overhead.
    ...
    \item[Redis-SET] Redis is a popular in-memory key value store.
        ...
        We configure it to happen every second.
        ...
        We enable AOF and configure \lstinline{appendfsync=always} which effectively turns Redis into a durable key-value store.

        As a benchmark, we use Redis's own \textit{redis-benchmark} tool~\cite{HowFastRedis}.
        We use the \textit{SET} workload to perform $10^6$ insert/replace operations with a random key drawn from a keyspace of the size $10^6$ to minimize contention within the daemon.
        We leave the values for pipelining and connection keepalive at their defaults (no pipelining, keepalive enabled).
        The scaling factor controls the number of parallel clients through the \lstinline{--threads} and \lstinline{-c} parameters.
        The result metric is the requests-per-second (rps) value reported by redis-benchmark.

        We chose the number of $10^6$ insert/replace operations because it results in runtimes of ten seconds or more per scaling factor value on all storage stacks except \textit{zfs-sync-disabled}.\todo{introduce storage stack names}
    ...
    \item[RocksDB-fillsync] RocksDB is a popular key-value store developed by Facebook that is optimized for fast storage devices.
        It can be used as an embedded database but is also used to implement DBMS storage engines such as MyRocks for MySQL.\todo{ref}
        ...
        Operations that request synchronous semantics (\lstinline{WriteOptions.sync}) are logged to a write-ahead log file.~\cite{FlushWALLessFwrite,RocksDBGitHubWikiWalPerformance}
        ...
        \textit{Fillsync} performs a fixed number of \textit{Put} operations with random keys in \lstinline{WriteOptions.sync} mode.
        ...
        We chose the number of 400k operations because it results in at least ten seconds of runtime for 90\% of configurations.
        The configurations that achieve less than ten seconds of runtime are:
        \begin{itemize}[noitemsep]
            \item All \textit{zfs-pmem} configurations at scaling factor $1$ (approx. \SI{4}{s} of runtime).
            \item \textit{zfs-sync-disabled} for scaling factors $1, 4, 8$ (approx. $2$, $6.8$ and \SI{8}{s} of runtime).
            \item \textit{xfs-on-devpmem} with the DAX mount optionat scaling factor $1$ (\SI{9.1}{s} of runtime).
        \end{itemize}
\end{description}
