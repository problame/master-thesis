...
ZIL-PMEM's ZIL header, which we describe in Section~\ref{ch:zilpmem}, is the second type in that union.
...
Afterwards, in Section~\ref{di:prb:approach}, we give a high-level overview of our approach.
Sections~\ref{di:prb:logstructure}, \ref{di:prb:replayapproach}, \ref{di:prb:deptrack}, \ref{di:prb:modeldatacorruption}, and \ref{di:prb:ccrecovery} progressively refine the mental model of an individual virtual log and how it is replayed.
Subsequently, we describe the storage substrate that maps the per-dataset virtual logs to the shared PMEM space.
...
Finally, in Section~\ref{di:prb:write}, we present the low-latency and CPU efficient design of the write path.
...
We use the same implementation to compute/re-compute counters on the write/recovery paths.
This works\todo{language} because the write path must use monotonically increasing generation numbers, ad the entries in the replay sequence are sorted in that order.
...
The PRB lifecycle starts during zpool import when we allocate it using the \lstinline{prb_alloc} function.
...
When freeing the PRB using \lstinline{prb_free}, the \lstinline{free_chunks} argument determines whether the PRB should \lstinline{chunk_free} the chunks or whether the ownership moves back to the caller.
...
void prb_teardown_hdl(hdl_t *hdl, bool abandon_claim, zil_header_pmem_t *upd);
\end{lstlisting}
...
Then, the API consumer can reset the header using \lstinline{zil_header_pmem_init} and re-setup the HDL using \lstinline{prb_setup_hdl}.
...
\begin{enumerate}[noitemsep]
    ...
    \item Load the entry $E$ into a DRAM buffer using \lstinline{prb_replay_read_replay_node(rn, ...)}.
...
\begin{lstlisting}
bool prb_create_log_if_not_exists(hdl_t *hdl, zil_header_pmem_t *upd);
...
After successful replay, the HDL is always in state \textit{nozil}.
We use the \lstinline{prb_create_log_if_not_exists} to start a new log idempotently.
...
If the header update is necessary, the caller must ensure that the transaction group that makes the changes has synced to disk before starting to write log entries.

We use the \lstinline{prb_write_entry} function to write log records to the HDL.
...
\chapter{ITXG Bypass For ZVOL}\label{sec:itxgbypass}
