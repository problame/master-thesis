...
The updated parts of the tree are stored in newly allocated disk blocks while unmodified subtrees re-use the existing blocks written in prior txgs.
...
This procedure is called \textit{txg sync} and is triggered periodically (default: every 5~s) or if the amount of dirty data exceeds a configurable threshold.
...
Instead ZFS maintains the \textbf{ZFS Intent Log (ZIL)} which is a per-filesystem logical write-ahead log.
The ZIL's \textit{records} describe the logical changes that need to applied to the filesystem in order to achieve the state that was reported committed to userspace.
On disk, the log records are written into a chain of \textit{log-write blocks} (LWBs), each containing many records.
The LWB chain is rooted in the filesystem's \textit{ZIL header} and is extended independently of txg sync.
...
However, when configuring the Linux PMEM block device as a SLOG in ZFS, a single thread only achieves 10k IOPS~(100~us/IOP), scaling up to 100k IOPS at 7 threads (70~us/IOP per thread).
...
We identify severe overheads caused by both the ZIL's physical data structure (LWBs) as well as the mechanism to persist it (ZIO pipeline).
...
Chapter~\ref{ch:lwb_analysis} describes our analysis of ZIL-LWB's sub-par performance on a PMEM SLOG device, motivating a new PMEM-specific ZIL implementation.
...
Chapter~\ref{ch:zilpmem} then describes how we integrate PRB/HDL into ZFS.
...
The last two sections on PMEM in Linux and ZFS provide the technical background knowledge that is necessary to understand the performance analysis of ZIL-LWB and the design of ZIL-PMEM in the subsequent chapters.
...
A \textit{log} or can be added to a pool in order to accelerate latency-sensitive I/O such as ZIL writes.
And \textit{aux} devices can be used as a victim cache for ZFS's primary DRAM cache.
...
Further, \textit{aux} devices reduce space efficiency because the cached data still occupies space in the main pool.
Similarly, \textit{log} devices are somewhat wasteful since they are exclusively used for ZIL logging and never read except during recovery after a crash.
Systems such as Strata derive more value from its PMEM-based log.
Finally, it should be noted that while Linux's /dev/pmem\todo{ref backwards} block device can be used with allocation classes, ZFS's ZIO pipeline is unable to exploit its low latency, as we will show in Section~\ref{ch:lwb_analysis}.
...
The evaluation is limited to a single benchmark: a fio 8k (\!) sync write workload, spread across four zpools in identical configuration, achieves 75k IOPS at four threads, which is a 4x speedup over the upstream implementation.
With 16 threads, the system achieves its peak IOPS at approx. 125k IOPS, which is a speedup of 2.5x.
The source code for the prototype has not been published and the design is incomplete with regards to replay.
...
Error handling code paths are notoriously difficult to test but often critical for correctness.
...
The ZIL's structure and its interaction with txg sync is substantially different from the journaling modes presented in the paper (See Sections~\ref{ch:openzfs_background:zillwb_persistence} and~\ref{di:prb:background}).\todo{review ref}
...
It is memory-mapped, byte-addressable, and accessible through regular CPU instructions, but, in contrast do DRAM, provides durability and significantly larger capacities.
In this section we provide the technical background on persistent memory that is required to understand the design and implementation of ZIL-PMEM.
...
In contrast, Optane PMEM uses the 3D-Xpoint non-volatile memory techonolgy as the primary data store, although on-DIMM DRAM may be used for caching \cite{yangEmpiricalGuideBehavior2020}.
% NVDIMM-P whose electrical and mechanical interface has recently been standardized as NVDIMM-P \cite{JEDECNVDIMM_P} ....

The performance of DRAM(/NVDIMM-N) and Optane PMEM differs significantly.
We summarize the insights of \citeauthor{yangEmpiricalGuideBehavior2020}\cite{yangEmpiricalGuideBehavior2020}.
...
\begin{description}[noitemsep]
    \item[Latency] The authors report 100~ns of random read latency (load instructions) for NUMA-local DRAM and ca 300~ns read latency for Intel Optane PMEM.
    ...
    \item[Scalability] PMEM and DRAM exhibit very different scalability with regards to access from multiple threads.
        Whereas DRAM bandwidth scales linearly and eventually plateaus, the Optane DIMM configurations' bandwidth declines after its peak.
...
``A	namespace	defines	a	contiguously­‐addressed	range	of	Non-­‐Volatile	Memory	  similar	 to	 a	 SCSI	 Logical	 Unit	 (LUN)	 or	 an	 NVM	 Express	 namespace.
[A] Persistent Memory namespace [\dots] provides	 a	 byte­‐addressable	 range	 of	 Persistent	 Memory	 in	 the	 system	 physical	 address	 space
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    \item[devdax] The namespace appears as a /dev/daxX.Y \textbf{character device} in the Linux devfs.
    \item[fsdax] The namespace appears as a /dev/pmemX.Y \textbf{block device} in the Linux devfs.
...
However, we use it in Section~\ref{ch:lwb_analysis} to evaluate raw device performance from userspace using \textit{fio}.

The \textit{fsdax} namespace is intended for \textit{\textbf{d}irect-\textbf{ac}cess} (DAX) capable Linux filesystems.
The Linux kernel provides a set of APIs to check whether a \lstinline[style=figurepseudocode]{struct block_device} is DAX-capable, i.e., an \textit{fsdax} namespace.
For such devices, the \lstinline{dax_direct_access} API allows for translation of a range of sectors (\lstinline{sector_t}) to a virtual address range.
...
Apart from internal optimizations, Ext4 and XFS in DAX mode support direct \lstinline{mmap}ping of individual files to PMEM.
This is possible because both filesystems overwrite data in-place --- supporting direct \lstinline{mmap} to PMEM is roughly equivalent to pre-allocating the mapped range's data ``blocks'' (=pages) on /dev/pmem and establishing a userspace-accessible memory mapping.
...
\subsection{PMEM Programming Model}
...
\begin{description}
    \item[Smaller Atomaticity Granularity] Instead of block-level atomicity, Intel platforms currently only guarantee atomicity for 8 byte sized and aligned stores.
        Unaligned writes or larger writes can be ``torn'' on power loss.
        In contrast, block devices such as NVMe drives provide more coarse-grained atomicity (e.g., one block) which can simplify the implementation of some data structures.
    ...
    \item[Durability] Stores to PMEM are not durable until they reach the system's \textit{persistence domain}.
        On current Intel platforms, this persistence domain does not include the processor caches.
        Until CPU caches become part of the persistence domain in future hardware revisions, it is thus necessary to flush CPU caches after issuing stores, or to use non-temporal store instructions, to achieve durability.
        \cite{rudoffPersistentMemoryProgramming2017}

    \item[Ordering] Due to pipelined, our-of-order, or speculative execution\todo{forgot sth?}, the program order of stores does not necessarily correspond to the order observed from outside the thread that issues them.
        ...
        Hence, for durability and ordering, the program must perform either issue \lstinline{store+cacheflush+sfence} or \lstinline{nontemporal_store + sfence} instructions.

    \item[Error Handling] The error handling story for PMEM is largely unsatisfactory.\todo{too harsh?}
        On the read path, PMEM can detect data corruption through ECC.
        It surfaces such problems and potentially other errors to the CPU which raises a \textit{machine check exception (MCE)}, which is the same mechanism that is used by DRAM to indicate ECC errors.
        The operating system can handle such exceptions by ``poisoning'' the affected memory ranges, i.e., unmapping the page and preventing re-use (``bad block list'').
        If the affected range is a userspace mapping, Linux notifies the application via SIGBUS.
        If the MCE is raised due to a kernel memory access, the default Linux behavior is to panic.
        However, the kernel provides the \lstinline{memcpy_mcsafe} function to handle MCEs more gracefully when copying from PMEM.
        Instead of panicking, the function returns an error if an MCE occurs.~\cite{ReliabilityAvailabilityServiceability,PersistentmemoryErrorHandling,xuNOVAFortisFaulttolerantNonvolatile2017}

        There is no mechanism to detect write errors since, even after a cache flush, the data may not have actually been written to the non-volatile media.
        ...
        The mitigation that Intel recommends against this problem is to perform memory scrubbing using a hardware engine (``patrol scrub'').
        The idea is to detect the corruption through hardware ECC.
        \cite{ReliabilityAvailabilityServiceability}
    ...
    \item[CPU Efficiency] Wait-time for PMEM I/O is spent on-CPU.
        ...
        To our awareness\todo{that doesn't seem to be a common phrase?}, we are the first to identify the above as a systematic issue, at least in the context of system services such as filesystems.\todo{qualifier yay nay?}
...
It is not used for any new DMU transactions, but some of the DMU transactions assigned to it may not yet have completed.\todo{may not have yet?}
...
\begin{figure}[H]
    \centering
    \includegraphics{fig/zil_writepath_activity_diagram_write_fsync_sync.pdf}
    \caption{
...
\begin{description}[noitemsep]
    ...
    \item[LWB Timeout / Group Commit]
        ...
        By sharing the last LWB of $A$ and $B$, up to $1 * lat_{disk}$ of waiting time can be avoided.\todo{review this}
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    \item[aux] A second-level victim cache for ZFS's primary in-DRAM cache, and/or hot spare devices.
...
\begin{itemize}[noitemsep]
    ...
    \item We create a zpool called \texttt{dut} with the 30 partitions as top-level vdevs, and the \textit{fsdax} namespace's /dev/pmem device as a SLOG.
        The reason for the large number of partitions is that we observed slightly improved \textit{txg sync} performance, presumably due to a higher degree oif parallelism towards the NVMe drives.
...
We use \textit{fio}\todo{ref} to generate a workload of random 4KiB writes (\lstinline{blocksize=4k}, \lstinline{rw=randwrite}).
...
To avoid scalability-bottlenecks in the ITX code, we place each thread's file onto a separate dataset \lstinline{/dut/ds$jobnum/fio_file} using the \lstinline{filename_format} option.
For each value of \lstinline{numjobs}, we measure for one minute (\lstinline{time_based=1}, \lstinline{runtime=60}), with a ramp-up time of 2~seconds (\lstinline{ramp_time=2}), and set \lstinline{end_fsync=1}.
...
For ZIL-LWB, small SLOG devices of \SI{16} to 32~GiB are sufficient in practice.\todo{ref ix systems truenas M}
...
\begin{figure}[H]
    \centering
    \includegraphics{fig/prb_writepath_crashconsistent_append}
    \caption{
...
The existing ZIL, which uses LWBs for persistence (cf. Section~\ref{ch:lwb_analysis:openzfs_background_zil_lwb}) becomes the first ZIL kind called \textit{ZIL-LWB}.
...
ZIL-LWB keeps its persistent state in the ZIL header that is stored in the \lstinline{objset_phys_t}.
...
In upstream ZFS, the ZIL runtime state is kept in the per-dataset object \lstinline{zilog_t}.
...
To accomplish this, we introduce a new allocation class called \lstinline{exempt} that is, by convention, never used for SPA allocations.\todo{maybe always allocate all of it so we can get rid of all the special cases?}
...
ZIL-PMEM DAX-maps\todo{ref linux intro?} the allocatable space of the \lstinline{exempt} SLOG VDEV and constructs the pool's PRB instance on top of it.
\begin{enumerate}
...
\textbf{Note:} Additional experience gained during implementation and testing has taught us that instead of attempting to exempt the PMEM SLOG VDEV's space from regular allocation, it is probably preferrable to actually pre-allocate all the PMEM space on the SLOG VDEV from the SPA.
...
The following cases required additional code to adapt betweent the two domains:
\begin{description}
    \item[Error Handling During Pool Import \& Claiming]
        We extracted the ZIL vtable from the original ZIL(-LWB), as described in Section~\ref{sec:zil_kinds:runtime}.
        Consequently, it inherits ZIL-LWB's model for integrity checking and claiming of the ZIL during pool import:
    ...
    \item[ZIL Header Updates]
        ...
        \lstinline{zilpmem_sync} then reads cell \lstinline{C := updates[syncing_txg&3]}.
        If \lstinline{C.txg == syncing_txg}, it updates \lstinline{objset_phys_t::os_zil_header} to \lstinline{C.header}.
        Otherwise, it does not modify it at all.
    ...
    \item[zil\_commit]
        ...
        \begin{enumerate}[noitemsep]
            ...
            \item Invoke the HDL's \lstinline{prb_write_entry} method for each ITX (i.e., log record) on the commit list.
                It starts a new generation for every ITX to encode the sequential log structure.
...
However, we also wanted to explore the performance potential of PRB/HDL if we eschew\todo{ok verb?} \textit{itxg} in favor of a design that allows for parallel writing of entries for the same dataset.
...
We use unit tests and integration tests to validate\todo{better verb?} our implementation.
...
\textit{libzpool} contains the majority of the ZFS kernel module's code --- it is used to by \textit{zdb} (the ZFS debugger) and the \textit{ztest} stress testing tool.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    \item[Independence of PRB \& Chunks]
        The testing code does not need to mock chunks or PRB.
        It also does not need to allocate the actual entries.
    ...
    \item[Independence of ZIL-PMEM]
        It is not necessary to mock or subsititute any behavior implemented in ZIL-PMEM.
...
\begin{description}[noitemsep]
    ...
    \item[ztest] The \textit{ztest} tool is a user-space binary that links against \textit{libzpool} and stress-tests core components of ZFS that are shared among all supported platforms.
        ...
        The stress-testing aspect comes from\todo{language} repeatedly dispatching these routines to a pool of threads.
...
\begin{description}[noitemsep]
    ...
    \item[File VDEVs] Both ZTS and ztest rely heavily on the \textit{file} VDEV type.
        ...
        For crash consistency, the memory mapping would further need to map directly to PMEM, i.e., be located on a Linux filesystem mounted with the \textit{dax} mount option.\todo{ref pmem in linux section}

    \item[VDEV Management]
        A significant number of tests in ZTS and ztest deals with\todo{language} mutations of the pool's VDEV config, e.g., adding, removing, growing, and offlining devices.
...
\begin{description}[noitemsep]
    ...
    \item[ZIL-LWB Specific Assertions] Some \textit{ztest} routines assert ZIL-LWB specific state.
        We move these assertions behind \lstinline{if} conditions that check whether the ZIL kind is ZIL-LWB and if so, downcast \lstinline{zilog_t} to \lstinline{zilog_lwb_t} to perform the check.
...
\end{enumerate}
The \lstinline|slog_replay_fs_00{1,2}| work directly on ZPL filesystems whereas \lstinline|slog_replay_volume| exercises the ZVOL code paths by instantiating a platform-native file system on top of the ZVOL (ext4 on Linux, UFS2 on FreeBSD).
...
\begin{description}[noitemsep]
    ...
    \item[Unified Storage Software Configuration] We abstract the setup and teardown of software storage systems such as ZFS or \textit{dm-writecache} as Python \textit{context manager} objects.
    ...
    \item[Unified Benchmark Abstraction] We abstract all benchmarks presented in this section as Python objects with a unified interface for benchmark execution.
...
We determine ZIL-PMEM's impact on real-world application performance through additional macro-level and application-level benchmarks.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    ...
    \item[dm-writecache]
        The \textit{dm-writecache} Linux Device Mapper target synchro\-nously persists writes to PMEM and performs asynchronous write-back to an origin block device in the background.
        We configure \textit{dm-writecache} with /dev/pmem0 as a cache device and a \textit{dm-stripe} of the 30 NVMe partitions as the origin data store.
        Since most benchmarks consume relatively little space compared to /dev/pmem0's capacity (40GiB), we configure dm-writecache's \textit{low watermark} to 0\% and \textit{high watermark} to 1\% in order to trigger some write-back during benchmark execution.
        (See Section~\ref{sec:cross_media_storage_systems} for details on \textit{dm-writecache}.)
    ...
    \item[zvol-pmem,rs=\{0,1\},byp=\{0,1\}] Like \textit{zvol-lwb}, but configured with the ZIL-PMEM ZIL kind.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    \item[ext4 on zvol-pmem,rs=1,byp=0] The Ext4 filesystem deployed on a ZVOL on a zpool with the ZIL-PMEM ZIL kind.
    ...
    \item[xfs-dax on devpmem] The XFS filesystem, deployed directly on the PMEM block device (/dev/pmem0).
...
In the \textit{MariaDB} benchmark, the DAX-aware Linux filesystem configurations achieve up to 1.57x more transactions per second than ZIL-PMEM.
...
Our first observation is that the default configuration of ZIL-PMEM, \textit{zvol-pmem,rs=0,byp=0}, delivers a significant speedup over ZIL-LWB.
...
To determine how the \textit{commit slot} abstraction impacts performance in ${\footnotesize\texttt{ncommitters}} > \max numjobs$ configurations, we add additional instrumentation that measures the average latency of commit slot aquisition and release.
...
\begin{figure}[H]
    ...
    \caption{
        Overhead of the \textit{commit slot} for $24 = {\footnotesize \texttt{ncommitters}} > \max numjobs$.
...
\begin{itemize}[noitemsep]
    ...
    \item Regarding the four-way interleaved configuration (4 Optane DIMMs):
        \begin{itemize}
            ...
            \item However, with ${\footnotesize\texttt{ncommitters}} > \max numjobs$, the overhead per IOP is negligible.
        \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item fault injection in ztest, equivalent to zio fault injection, should be easy
    \item fault injection through ndctl-inject for kernel module \& ZTS: Although we force the replay callback to use the \lstinline{prb_replay_read_replay_node} function which handles \textit{machine check exceptions (MCE)} through \lstinline{memcpy_mcsafe()}, we should somehow ensure that such errors are actually handled.
\end{itemize}
...
\begin{description}
    ...
    \item[Fuzzing] 
    \item[ZTS]  run entire ZTS, not just ZIL-related tests
    \item[] The Dirty Shutdown Handling guide describes the (hopefully rare) error case where flushes to persistent memory did not go as expected on power failure or system crash.  The guide describes how this failure is detected, and how it is communicated to applications via the dirty shutdown count.
\end{description}
