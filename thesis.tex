...
The counters are stored in a table with four rows $R_i := (txg, ctr), i \in \{0,1,2,3\}$.
When an entry $E$ is written to the HDL for txg $T := E.txg$, it modifies the counter in the row with index $I_T := T \mod 4$.
...
To detect the start of a new generation, we maintain a separate variable $open\_gen$.
When an entry $E$ is written to the HDL, we compare $E.gen$ and $open\_gen$.
The case $E.gen < open\_gen$ is prohibited because the API contract for log writers is that generation numbers must be monotonically increasing.
If $E.gen = open\_gen$, the writer did not start a new generation and we leave the \textit{last table} unmodified.
If $E.gen > open\_gen$, we snapshot \textit{live table} and replace the \textit{last table} with the snapshot.
Then, we update $open\_gen \leftarrow E.gen$ and increment the counter in the \textit{live table} as described in the previous paragraph.
...
\begin{itemize}[noitemsep]
    ...
    \item Its \textit{txg} value must be in the correct corridor of unsynced txgs that.\todo{how to explain corridor?}
\end{itemize}
