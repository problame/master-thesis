...
\usepackage{floatpag} % sudo dnf install texlive-sttools
% needs to be imported before package float https://tex.stackexchange.com/questions/277913/floatpag-package-conflict-with-float
...
\def\declaration@date{June 7, 2021}
...
Persistent memory (PMEM) is an emerging technology that provides low-latency mem\-ory-mapped byte-addressable persistent storage.
The \lstinline{/dev/pmem} pseudo block device allows existing block device consumers to benefit from PMEM's high throughput and low latency without modification.

To explore the use of PMEM as a storage medium for the ZIL, we configure a ZFS storage pool that uses \lstinline{/dev/pmem} as a SLOG.
We find that the current ZIL implementation (ZIL-LWB) shows significantly higher latency and sub-par throughput compared to the raw PMEM hardware in a 4k synchronous random write workload.
...
Applications operate on files through system calls such as \lstinline{write()} which generally do not provide any durability guarantees.
Instead, these system calls only modify DRAM buffers, such as pages in the Linux page cache, and return to userspace.
...
In the event of a crash and subsequent reboot, the accounting system would then load this outdated state from disk, enabling double-spending by the account holder.
...
By default, ZFS allocates LWBs from the zpool's primary storage devices.
To address the case where these devices' latency is insufficient, ZFS provides the ability to add a \textit{separate log device} (SLOG) to a zpool which is reserved and preferred for LWB allocation.
A typical configuration is to use a single or mirrored NVMe drive(s) as a SLOG in an HDD-based zpool.
Note that the space requirements for the ZIL (and thus SLOGs) are low because LWBs are garbage-collected as soon as txg sync has written out the dirty state of all log records that an LWB contains.
...
The motivation for this thesis is to accelerate synchronous IO in ZFS by using PMEM as a SLOG device.
A single DIMM of the current Intel Optane DC PMEM product line can sustain 550k random 4k write IOPS from a single CPU core, corresponding to a write latency of 1.81~us.
...
We find that approximately 80\% of wall clock time for the average synchronous write IOP is spent in the ZIL code, and we conclude that both the LWB structure and the mechanism to persist it (ZIO pipeline) are unfit for PMEM-level performance.
...
With ZIL-PMEM, a single thread achieves 128.5k random 4k synchronous write IOPS and scales up to 400k IOPS at four threads.
Our implementation is extensively unit-tested and passes the ZFS test suite's SLOG integration tests.

The remainder of this thesis is structured as follows:
in Chapter~\ref{ch:litreviewandbackground} we review prior work in the field of filesystems that use persistent memory, and provide technical background knowledge on PMEM and ZFS.
...
It batches log records issued by independent threads for the same filesystem into the currently ``open'' \textit{log-write block} (LWB).
...
However, since ZIL-PMEM shall maintain the same guarantees as ZIL-LWB (Section~\ref{sec:requirements}), such findings would only be relevant for future work.
...
The authors pre\-sent an “implementation-level model checker” that removes the need to define a formal model for the filesystem.
...
Thus, as soon as the syncing txg has been fully written, the quiescing txg becomes the new syncing txg, the open txg becomes the new quiescing txg, and a new open txg is born.
...
After a system crash, the zpool is in the state of the last synced transaction group, which we refer to as $precrash\_txg$.
...
\begin{description}[noitemsep]
    ...
    \item[LWB Timeout / Group Commit]
        ...
        If no other thread picks up $L_n$, a timeout wakes up $A$ so that it can issue the IO operation itself.
...
As hinted in Figure~\ref{fig:zil_lwb__physical_structure_overview}, a typical configuration is the use high-capacity HDDs in a space-efficient \textit{raidz} configuration for the main pool and a costlier but faster NVMe drive, or mirror thereof, as a SLOG.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    ...
    \item[async] The same configuration as above, but with \lstinline{sync=disabled} set on all data\-sets.
        This setting effectively disables the ZIL and thereby violates the synchronous semantics.
        It serves as an upper bound for the performance that any ZIL implementation can achieve.
        The remaining work done by \textit{async} is only CPU- or DRAM-bound, i.e.,  DMU object modification and ITX allocation.
    \item[fsdax] The fio configuration above, but applied directly to the /dev/pmem0 block device instead of the filesystem, using \lstinline{direct=1,filename=/dev/pmem0} instead of \lstinline{direct=0} and the \lstinline{filename_format} option.
...
Further, ZIL-PMEM should be integrated into the \textit{ztest} user\-space stress test as well as the SLOG tests of the ZFS Test Suite.
...
Sections \ref{di:prb:deptrack}, \ref{di:prb:modeldatacorruption}, and \ref{di:prb:ccrecovery} then progressively refine our understanding of replay and explain how data corruption and crash consistency is addressed by HDL, culminating in a comprehensive example in Section~\ref{di:prb:comprehensiveexample}.
...
Section~\ref{di:prb:gc} explains how garbage collection removes obsolete entries from PRB.
...
\end{description}
We always update \mbox{$E_{last} \leftarrow E$}, regardless of whether a new generation was started or not, and increment the counter in the \textit{live table} as described in the previous paragraph after evaluating the rules above.
...
Suppose that during initial claiming, entries $(42,1)$ and $(42,3)$ in generation $42$ are observed as the last entries of the replay sequence.
In that case, $(E_{seal}.gen, E_{seal}.gsid)$ is $(43,1)$ and the counters of $E_{seal}$ only account for these two entries, and all previous generation's entries.
...
\section{HDL: Comprehensive Example For Replay}\label{di:prb:comprehensiveexample}

In this section, we present a comprehensive example of the replay algorithm.
Figure~\ref{fig:prb_replay_full_example} visualizes
\begin{itemize}[noitemsep]
    \item how $precrash\_txg$ controls which entries the algorithm considers for replay,
    \item how $E_{last}$ causes already-replayed entries to be skipped after a crash (``last replayed'' in the figure), and
    \item how we detect lost entries through the counters that the write path stores in each entry.
\end{itemize}
Due to the shortcomings of $E_{seal}$ that we mentioned in the previous section, we do not cover it in the example.
However, the reader may imagine that entry "L" is $E_{seal}$ to get an idea of its effect.

\newgeometry{includehead,includefoot,top=0.75cm,bottom=0.75cm,headsep=0.25cm,textwidth=1.2\textwidth}
\begin{figure}[p]
    \centering
    \includegraphics[trim=0cm 0.3cm 0cm 0.3cm,clip]{fig/prb_replay_full_example.pdf}
    \caption{Comprehensive example of the replay algorithm.}
    \label{fig:prb_replay_full_example}
\end{figure}
\restoregeometry
...
The PRB scan used by claiming and replay is an iteration over the entries of all chunks of the PRB.
...
\begin{description}[noitemsep]
    ...
    This rule also applies to subsequent garbage collection cycles so that the write path can re-use a chunk after all of its holding HDLs have finished replay.
    ...
    Suppose we would garbage-collect chunks immediately after the last holding HDL has finished replay. If we crashed before the on-disk ZIL header has transitioned from state \textit{replaying} to \textit{nozil}, the garbage-collected chunk's entries would be missing when resuming replay after the crash.
\end{description}

Figure~\ref{fig:prb_chunk_ownership_cycle__transitions} visualizes the possible transitions of a chunk over its lifetime and Figure~\ref{fig:prb_chunk_ownership_cycle__example} provides an example.

\begin{figure}[H]
    \centering
    \includegraphics[trim=0cm 0.3cm 0cm 0.3cm,clip]{fig/prb_chunk_ownership_cycle__transitions}
    \caption{The different owners of a chunk over its lifetime and the events that cause ownership transitions.}
    \label{fig:prb_chunk_ownership_cycle__transitions}

    \vspace{1cm}

    \centering
    \includegraphics[trim=0cm 0.3cm 0cm 0.3cm,clip]{fig/prb_chunk_ownership_cycle__example}
    \caption{
...
\begin{description}
    ...
    \item[Header: Detected Data Corruption]
        If header checksum validation fails, the header was either corrupted or never completely written.
...
\begin{description}[noitemsep]
    ...
    \item[Commit Slot Acquisition \& Chunk Replacement]
        Both the commit slots and the chunk lists are shared among all HDLs.
    ...
    \item[PMEM Bandwidth Limits \& Multicore Scalability]
        ...
        For example, instructions that architecturally depend on a preceding write to PMEM (store+cacheflush+sfence) will stall until the flushed cache line reaches the power-fail protected domain of the CPU~\cite{Scargall2020}.        This is problematic from a CPU utilization perspective:
        ...
        Therefore, our design must find a way to limit concurrent access to PMEM and shift wait time off the CPU.
\end{description}
...
To abaondon the log, the API consumer must first tear down the HDL using \lstinline{prb_teardown_hdl} with argument \lstinline{abandon_claim=true} to release claims made during \lstinline{prb_claim}.
...
Note that the callback is forced to use \lstinline{prb_replay_read_replay_node} function because \lstinline{replay_node_t} is an opaque type in the PRB API.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    ...
    \item[needs\_new\_gen] Indicates whether a new generation should be started for this log entry.
        ...
        If a thread writes an entry with \lstinline{needs_new_gen} is \lstinline{true}, that thread must be the only thread that is executing \lstinline{zilpmem_prb_write_entry} for the given HDL.
        The contrary is not true: multiple threads may write entries in parallel to the same HDL if they set \lstinline{needs_new_gen} to \lstinline{false}.
...
For example, ZIL-LWB only operates on the \lstinline{zil_header_lwb_t} structure, not \lstinline{zil_header_t}.
...
As a stop-gap solution, we instead implemented a simplified scheme where a zpool's ZIL kind is determined on pool creation time by a kernel module parameter called \lstinline{zil_default_kind}.
...
\begin{itemize}[noitemsep]
    ...
    \item The PMEM SLOG's space must be directly accessible via the PMEM programming model (load/store instructions, cache flushing, etc., see Section~\ref{sec:background:pmemprogrammingmodel}).
...
Instead, the kernel module parameter \lstinline{zil_default_kind} determines the ZIL kind of a newly created pool (ref. Section~\ref{sec:zil_kinds:change}).
...
\begin{itemize}[noitemsep]
    ...
    \item When a new head dataset is created we set up its HDL and add it to \textit{HDL map} (\lstinline{dmu_objset_create_impl_dnstats} and \lstinline{dmu_objset_clone_sync}).
...
\begin{description}
    \item[zil\_commit]
        ...
        \begin{enumerate}[noitemsep]
            ...
            \item Write each ITX (i.e., log record) on the commit list to the HDL using the \lstinline{prb_write_entry} function.
    ...
    \item[ZIL Header Updates]
        ...
        Each cell of the array contains the tuple \lstinline{(txg: u64, header: zil_header_pmem_t)}.
...
\begin{description}[noitemsep]
    ...
    \item[Same Guarantees] ZIL-PMEM maintains the same crash consistency guarantees as ZIL-LWB towards userspace, courtesy of the shared logical structure and guarantees provided by PRB/HDL's.
\end{description}
...
With regard to PRB, an entry $E_i$ must only be written to PMEM if its txg $E_i.txg$ is newer than the HDL's highest-ever written txg $T_{max}$, or if it is one of $T_{max}$, $T_{max}-1$, or \mbox{$T_{max}-2$}.
...
\end{enumerate}
Whereas \lstinline|slog_replay_fs_00{1,2}| work directly on ZPL filesystems, the \lstinline|slog_replay_volume| test exercises the ZVOL code paths by instantiating an external filesystem on top of the ZVOL (ext4 on Linux, UFS2 on FreeBSD).
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    ...
    \item[zfs-pmem] ZFS with the ZIL-PMEM ZIL kind and the default value of \lstinline{3} for \lstinline{ncommitters}.
        The \lstinline{zvol_request_sync} option and the ITXG bypass are only relevant for ZVOLs and therefore are not part of the configuration name.
\end{description}
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    \item[Scaling Factor] The scaling factor increases the degree of concurrent synchro\-nous IO operations issued by the benchmark.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    ...
    \item[filebench varmail] Filebench is a benchmark execution engine that allows for the definition of workloads in a domain-specific language.
        ...
        We compile filebench from source (\lstinline{1.5-alpha1-33-g22620e6}) and use the upstream workload definitions.
...
We compare the performance of XFS deployed on a ZVOL in a zpool with varying ZIL kinds, values for \lstinline{zvol_request_sync} (\lstinline|rs_{0,1}|), and ITXG bypass setting (\lstinline|byp_{0,1}|).
...
PRB's \textit{commit slot} mechanism limits the number of parallel writers to \lstinline{ncommitters}.
...
But with \lstinline{ncommitters=8}, the system is able to sustain the 400k IOPS for the for \mbox{$numjobs \in 7 \dots 13$}.
...
Our main contribution is the PRB/HDL data structure which consumes the space of a PMEM SLOG device and exposes the abstraction of a persistent garbage-collected log (HDL) for each dataset in the pool.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    ...
    \item[XFS DAX Log Patch] XFS developer Christoph Hellwig has published an unfinished patch that adds DAX support for the code in XFS that writes its write-ahead log~\cite{LKMLChristophHellwig_linebreaks}.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    ...
    \item[Fault Injection for PMEM Access] Whereas we are careful to always use the \lstinline{memcpy_mcsafe} function when accessing PMEM, we do not have tests that verify the error handling path.
...
\begin{description}[noitemsep,leftmargin=1.5cm,labelindent=1cm]
    ...
    \item[Redundancy Through Mirroring] Redundancy for ZIL-PMEM could be accomplished by mirroring PRB between two or more DIMMs.
...
The branch \lstinline{master-thesis-archive/submitted_state} at commit \\ a467b56f3ca60d60e9eb66967f4c8dcfbb44c99c contains:
\begin{itemize}[noitemsep]
