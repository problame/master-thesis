...
It acts as a resource manager that multiplexes PMEM among many virtual logs (HDLs).
The \textit{zil\_pmem.c} module (Section~\ref{sec:zilpmem}\todo{chapter}) uses HDLs to implement the ZIL-PMEM ZIL kind.
...
In Section~\ref{di:prb:background} we recapitulate the role of the ZIL in ZFS to isolate the requirements that the ZIL-PMEM ZIL kind will impose\todo{verb} on PRB.
Based on these requirements we develop a model of the log abstraction that PRB exposes through its HDLs (Section~\ref{di:prb:structure}).
In Section~\ref{di:prb:api} we describe the lifecycle of PRB's abstractions by walking through the API exposed toward \lstinline{zil_pmem.c}.
...
Remember from section \ref{openzfs:pool_operation} that whenever a file system operation changes a dataset $D$, it does so in a DMU transaction $T_i$ within a transaction group $T_{i_{txg}}$.
When a system call handler invokes \lstinline{dmu_tx_commit(T_i)} and subsequently returns to userspace, the logical change $C_i$ that $T_i$ makes is not yet persisted to stable storage.
Instead, the DMU accumulates the changes from many DMU transactions in DRAM, grouped by the transaction's txg.
Eventually, the \textit{txg sync} background thread syncs the txg out to the zpool's storage devices.
...
After this process is complete, the transaction group is the pool's \textit{last synced} txg.
Note that there are three unsynced txgs at every given time, one for each of the three states \textit{open}, \textit{quiescing}, and \textit{syncing}.
...
The purpose of the ZIL is to bridge the gap between the time at which a DMU transaction $T_i$ is committed and the time at which the changes made by $T_i$ actually reach the main pool through \textit{txg sync}.
For that purpose, the system call handler encodes the change $C_i$ that was made in $T_i$ as a \textit{log record} and \textit{assigns} it, wrapped in an ITX, to the ZIL.
If the system call has synchronous semantics, the handler calls \lstinline{zil_commit} before returning to userspace.
\lstinline{zil_commit} uses the \lstinline{itxg} data structure to determine which log records need to be written out, resulting in a \textit{commit list}.
At this point, the ZIL-kind specific implementation takes over: it must append the commit list to the previous commit lists and ensure that that this concatened list is persisted to stable storage.

After a system crash, the zpool is in the state of the last synced transaction group which we call \textit{precrash-txg}.
Our dataset $D$ is lacking exactly those changes $C_i$ whose transactions $T_i$ were made in txg $T_{i_{txg}} > \text{precrash-txg}$.
When $D$ is mounted, the mount procedure calls \lstinline{zil_replay} to apply those changes $C_i$ to $D$.
The implementation of \lstinline{zil_replay} is specific to each ZIL kind but the API contract is the same:
the caller provides a callback that the implementation invokes for every log record that represents a missing change.
The callback interprets the log record and performs a DMU transaction that atomically applies the change to the dataset and records replay progress.
The invocation order is given by the commit list, but entries for transactions before or in \textit{precrash-txt} must be skipped because the change encoded in them is already part of the dataset state.

\subsection{Model}\label{di:prb:structure}

On the write path, ZIL-PMEM will need to append entries to a sequential log that is synchronously persisted to stable storage.
In the event of a crash, ZIL-PMEM needs to recover the exact subset of log records that represent changes that have not been synced to the main pool.
Since the log is written on a per-dataset basis but PMEM is treated as a pool-wide shared resource, some abstraction is needed to mediate access to PMEM in a multicore-scalable manner.

In our design, PRB assumes the role of this mediator.
A PRB is set up with many segments of PMEM that are used for storing log data.
For each dataset, it exposes a virtual log in the form of the HDL abstraction.
HDLs share the same PMEM space but can be written and replayed independently.

A HDL log can be opened once and written until the system crashes or the log writer destroys it, e.g., because the dataset is being unmounted.
After a system crash, HDL provides the replay interface described in the previous section: a callback is invoked for exactly those changes that were made after the \textit{precrash-txg}.

The unit that is written to a HDL is the \textit{log entry}.
A log entry is the representation of a logical change $C_i$ that was made in a DMU transaction $T_i$ within a transaction group $T_{i_{txg}}$.
The consumer of PRB encodes $C_i$ as an opaque blob and passes it to its HDL's \lstinline{write_entry} method.

The logical structure of a HDL log is that of a sorted sequence of \textbf{generations}.
When an entry is written, the writer must indicate whether the entry needs to be placed in a new generation.
Generations encode logical dependencies between log entries.
...
The logical dependencies are checked during replay: if an entry $E$ depends on an entry $E'$ that is missing and $E'$'s txg is younger than the precrash-txg, replay stops with an error at entry $E$.

To structure the HDL log as a fully sequential list, as required by the ZIL's \textit{commit list} model, a new generation must be started for every entry.
However, the inherent sequentiality of the \textit{commit list} model presents an unnecessary scalability bottleneck.
For example, if multiple threads modify the same file simultaneously, their modifications cannot be causally dependent and could therefore be logged in parallel without coordination.
HDL addresses this problem by allowing log entries for the same generation to be written in parallel.
We present an application of parallel logging to ZVOLs in Section~\ref{sec:itxgbypass}.
Our evaluation in Section~\ref{ch:eval}\todo{precise} demonstrates that significant speedups are possible without weakening crash consistency guarantees.

We provide an example of the HDL's structure and the effect that generations have on replay in Figure~\ref{fig:prb_logical_structure}.
...
\begin{figure}[H]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \caption{
            Entries 2 and 3 were written in the same generation.
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[height=5cm]{fig/prb_logical_structure__diamondchain2}
        \caption{
    ...
    \caption{
        ...
        The \textit{precrash-txg} is represented as a horizontal line.
...
In this subsection we provide a walkthrough of that API to illustrate the lifecycle of the PRB and HDL.
...
The callback must perform the following steps in an atomic DMU transaction:
\begin{enumerate}[noitemsep]
...
The callback does not access PMEM directly to access the log entry but uses the \lstinline{zilpmem_prb_replay_read_replay_node} function instead.
