...
In Section~\ref{di:prb:phys} we describe PRB's persistent data structures and their invariants.
...
\subsection{Logical Structure}\label{di:prb:structure}
We model the log that is written and replayed through a HDL as a \textbf{sequence of generations of log entries}.
\begin{description}
    \item[Log Entry] A log entry consists of a variable-length opauqe body provided by the log writer and a fixed-length header.
        When writing a log entry the PRB API consumer must provide the transaction group in which the entry becomes obsolete.
        It must also indicate whether the entry should start a new generation.
    \item[Generation] Generations are used to express logical dependencies between entries.
        ...
        But an entry from a later generation always depends on all log entries in all previous generations.
        The replay algorithm chooses a replay order that respects these dependencies and refuses to replay an entry if one or more of its dependencies is missing, e.g., due to data corruption.
...
An entry that does not logically depend on the other entries in the open generation can be written to it as well.
...
If one of the two log entries is lost, the other can still be replayed.
In contrast, if a log entry is lost in the sequential chain, none of the subsequent entries can be replayed.
...
However, as pointed out by the example in the previous paragraph, full sequentiality is not always necessary.
...
Separating these concerns allows us to optimize the write path for latency at the expense of more but tolerable complexity during recovery.

\subsusbsection{Chunks}
The PMEM chunks that are added to the PRB during setup are the storage location for log entries.
Chunks have a base address and a length.
The base address must aligned to 256 bytes and the length in bytes must be a multiple of 256.
We store entries as a contiguous sequence, starting with the first entry's header at the chunk's base address.
The fixed-length entry header is followed by the variable-length body and zero padding to the next multiple of 256 bytes which is where the next entry header starts.
The sequence is terminated either explicitly by an invalid header or implicitly if the last entry's body and padding have filled the chunk completely.

\subsubsection{Entry}
Log entries are represented as a fixed-length header and a variable-length opaque body.
...
The entry header stores metadata to allow claiming and replay to
\begin{itemize}
    \item iterate over the entry sequence stored in the chunk,
    \item ensure data integrity of the entry's body,
...
We attribute entries to a given HDL's log through the \ltextit{log GUID}.
\begin{description}
...
\begin{description}
    \item[Transaction Group (txg)] The transaction group in which the entry's effect was to be synced out by \textit{txg sync}.
    ...
    \item[Counters] The number of entries that have been written up to generation \lstinline{gen}, by transaction group.
        Only the counters for transaction groups \lstinline{txg}, \lstinline{txg-1}, \lstinline{txg-2} are stored.
\end{description}
We describe the algorithm for claiming and replay in Section~\ref{di:prb:recovery}.


\subsubsection{ZIL header}
The ZIL header (\lstinline{zil_header_pmem_t}) stores the HDL's persistent state.
It is an opaque struct to PRB API consumers.
Internally, it is represented as \lstinline{zil_header_pmem_impl_t}.
...
The persistent state of PRB consists of three entities:
\begin{description}
    \item[ZIL Header] The fixed-size \lstinline{zil_header_pmem_t} structure that contains the HDL's persistent state.
    \item[Entry] The representation of a log entry. It consits of a fixed-size header (\lstinline{entry_header_t}) and a variable-length opaque body.
...
The chunk base address must be 256 byte aligned.

Since entries are variable-sized we 
Each chunk contains a contiguous sequence of as contiguous sequences that are terminated by an invalid entry header.

 a we give maximum flexibility to the write path with regards to placement of  can allow parallel writers 
We use the PMEM chunks as a 
On the write path, the writing thread appends the entry to   chooses a PMEM chunk and We store log entries in the PMEM chunks as contiguous sequences that are terminated by an invalid entry header.
 Replay relies solely on metadata in the entry headers to reconstruct the logical log structure.
This approach enables multicore-scalability on the write path because parallel writers can append to separate chunks without coordination.


PRB's persistent state consists of the following entitites:
\begin{description}
    \item[ZIL Header] The fixed-size \lstinline{zil_header_pmem_t} structure that contains the HDL's persistent state.
        As explained in the API overview (Section~\ref{di:prb:api}) \lstinline{zil_header_pmem_t} must be persisted to the main zpool by the PRB API user.
    \item[Entry] The representation of a log entry. It consits of a fixed-size header (\lstinline{entry_header_t}) and a variable-length opaque body.
    \item[PMEM chunks] The segments of contiguous PMEM space that are added to the PRB during setup (see Section~\ref{di:prb:api:setup}).
\end{description}


Figure~\ref{fig:prb_physical_data_structure_entities} visualizes the relationships described above.

\begin{figure}
    \includegraphics[height=10cm]{fig/prb_physical_data_structure_entities}
...
\subsubsection{Chunk Layout}




This gives us the freedom to scatter entries over choose 
Entries are stored in chunks as a contiguous sequence that is terminated by an invalid header.

that an entry's physical location in PMEM has no semantic meaning.
 of the data structure is that we decouple a log entry's location in PMEM in the  logical structure  logical chain of entries is never represented an entries are linked into the logical chain.
...
in a way that suppo meets the requirements outlined in Section~\ref{sec:requirements}.
...
in a way that suppo meets the requirements outlined in Section~\ref{sec:requirements}.
Given the logical structure and API of PRB, we can formulate these requirements more precisely:
...
%Parallel writing to separate HDLs should result in a near-linear speedup.
%Parallel writing to a single HDL (same generation) should result in significant\todo{...} speedup on a single-socket system.
% \item[CPU-Efficient Handling Of PMEM Bandwidth Limits] 
% We identify the following access characteristics:
% \item[Crash Consistent Replay] Entries Replay must be crash consistent.

Write-mostly.



used by that represent the representation to The physical representation of PRB has the following requirements:

Although we provide the abstraction of a mostly sequential log to PRB API consumers, the physical 

PRB purpose of stores multiplexes 
The logical structure of a single HDL's log (see Section~\ref{di:prb:structure}) is very different from its representation in persistent storage.
Whereas the individual logs are written in a mostly sequential manner, PRB must multiplex many log writers the 
whereas a single  more than an ordered sequence of log entries, sorted sequence 
very reminiscent of the ZIL chain, simple  is very different from its representation in persistent storage.
The persistent state of PRB consists of three entities:
\begin{description}
    \item[ZIL Header] The fixed-size \lstinline{zil_header_pmem_t} structure that contains the HDL's persistent state.
    \item[Entry] The representation of a log entry. It consits of a fixed-size header (\lstinline{entry_header_t}) and a variable-length opaque body.
...
chain is uniquely identified by a 128bit random number that is stored in the HDL's \lstinline{zil_header_pmem_t} and every entry written through the HDL.

Each chunk contains a contiguous append-only sequence of entries, terminated by an invalid header.
...
We store entries in the PMEM chunks as an append-only contiguous sequence terminated by an invalid header.

The position of the entry within the chunk nor its location in PMEM space is semantically relevant.
Instead, we store sufficient metadata in the entry headers so that the logical log structure (see Section~\ref{di:prb:structure} can be reconstructed during claiming and replay.


    \item[Generation] We represent generations as unsigned 64-bit non-zero integers.
        When a log is created the generation number starts at $1$.
        When an entry is written, the log's current generation number is stored in the entry header.
        If the log writer requests that the log entry be placed in a new generation, the generation number is incremented.
        There are no provisions for wrap-around.
    \item[Generation-Scoped ID] Within a generation, we uniquely identify entries by another unsigned 64-bit non-zero integer called \textit{generation-scoped-id} (GSID).
...
\subsubsection{Entries \& Chunks}



\subsusbsection{Entry Sequence In A Chunk}

\subsubsection{ZIL Header}

The ZIL header \lstinline{zil_header_pmem_t} is opaque to the PRB API consumers.
Internally, it is represented as \lstinline{zil_header_pmem_impl_t}.
...
The ZIL header can be in one of the following states:

nozil: The PMEM space does not contain any entries for this dataset.
zil_guid = 0
claim txg range is [0, 0]
replay position is {0}
logging: The PMEM space might contain unclaimed entries. The purpose of this state is to let claiming know that there might be entries in pss_ring that should be claimed for this dataset.
zil_guid != 0
claim txg range is [0, 0]
replay position is {0}
claimed: We claimed all entries for this objset. The purpose of this state is to keep the claimed entries with entry.txg ∈claim_txg_range alive until we have replayed them. 
zil_guid != 0, same as it was in state logging
claim txg range is [i, j] with i != 0 and j != 0.
replay position is {0}
replaying: We are replaying entries of this objset. The purpose of this state is to track replay progress in the replay position member in case we crash during replay.
zil_guid != 0
claim txg range is as in claimed
replay position is { .txg, .seq } with txg ∈ claim_txg_range and seq != 0, identifying the where the replay algorithm should resume replay.

The following ZIL header transitions exist:

Regular:
nozil => logging zil_create()
logging => claimed after claiming is done
claimed => replaying after the first txg in which replayed has synced
replaying => logging after replay is done
{replaying,claimed,logging} => nozil: zil_destroy()


Checkpoint-related:
replaying => claimed if the pool is rewound to a checkpoint and the dataset was claimed but not replayed at the time of the checkpoint
Not a problem because the presence of the checkpoint kept the entries that were claimed live during the time that the pool moved on with the checkpoint present. (Liveness is explained in the Claiming & Replay section)
{replaying, claimed} => logging if the pool is rewound to a checkpoint and the dataset was logging at the time of the checkpoint
After we rewind to the checkpoint, spa_checkpoint_txg is zero and the spa_last_synced_txg is greater than the txg of any entry that could have been present at the time of checkpoint (because the rewound uberblock gets a txg that is > the txg of the txg before the rewind so that is selected as best uberblock on import after rewind). 



a log entry's physical location in PMEM is irrelevant.

Log entries Log entries are attributed to  HDL through a random 128bit identifier that is stored in the ZIL header and repeated in every entry header.
Each log entry header contains sufficient stores sufficient ordering information 
